...
function generateIndustryStandardReceipt(txData) {
  ...
  doc.setTextColor(...ACCENT_COLOR);
  doc.text(
    `₹${parseFloat(txData.amountPaid || 0).toLocaleString('en-IN')}.00`,
    190, finalY + 8.5,
    { align: 'right' }
  );
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div id="pastDueForm" style="pointer-events:none;opacity:0.4">
            ...
            <button class="btn btn-primary btn-full" onclick="_pastDueSave()">
...
async function _pastDuePick(sid) {
  ...
  if (_pdCtxEl) {
    ...
    if (_prevYrB && _prevYrB !== _currentAcadYrNorm) {
      const _prevRateB = _FEE_SCHEDULE[s.classPrev || s.openingOutstandingClass || s.class || s.cls] || _monthlyFeeB;
    ...
    if (Array.isArray(s.openingOutstandingDues)) {
      s.openingOutstandingDues.forEach(d => {
        ...
        const _dRate = _FEE_SCHEDULE[d.class || s.class || s.cls] || _monthlyFeeB || 0;
    ...
    if (_openingYrB && _openingYrB !== _currentAcadYrNorm && !_bannerYearMap[_openingYrB]) {
      const _openRateB   = _FEE_SCHEDULE[s.openingOutstandingClass] || _monthlyFeeB || 0;
    ...
    for (const cand of _bannerCandidateYears) {
      ...
      try {
        ...
        const _liveRate = _FEE_SCHEDULE[cand.cls] || s.monthlyFee || 0;
  ...
  const curStart = now.getMonth() >= 5 ? now.getFullYear() : now.getFullYear() - 1;
  ...
  (async () => {
    ...
    if (studentYearsSet.size === 0) {
      ...
      const _yCur = _yn.getMonth() >= 5 ? _yn.getFullYear() : _yn.getFullYear() - 1;
      for (let i = 1; i <= 2; i++) {
        const ys = _yCur - i;
...
async function _pastDueLoadMonthGrid(year, sid, openingMonths, studentData) {
  ...
  let paid = new Set();
  const _appliedPD = {};   // JSS-REF-VELTRIX-2026-004 ITEM 06: per-month amount applied via partial txs
  ...
  try {
    ...
    txSnap.docs.forEach(d => {
      ...
      // JSS-REF-VELTRIX-2026-004 ITEM 06: capture per-month applied amounts (from partial
      // payments) so a PARTIAL month can be kept selectable below instead of locked green.
      if (data.monthAllocations && typeof data.monthAllocations === 'object') {
        Object.entries(data.monthAllocations).forEach(([m, amt]) => { _appliedPD[m] = (_appliedPD[m] || 0) + (Number(amt) || 0); });
      }
    });
  ...
  const _pdGridRate = _FEE_SCHEDULE[_resolveClassForYear(sData, year)] || sData.monthlyFee || 0;
  // Cache per-month applied amounts so _pastDueCalcAmount can net a PARTIAL month's remainder
  // (rate − applied) when it's tapped — same source (monthAllocations) as the grid + banner, so
  // the pre-filled amount, the outstanding, and the recorded payment never disagree.
...
function _pastDueCalcAmount() {
  ...
  const rate = (_FEE_SCHEDULE[prevCls] || 0);
...
async function _pastDueSave() {
  ...
  const rate   = _FEE_SCHEDULE[cls] || 0;
  ...
  const tx = {
    ...
    // JSS-REF-VELTRIX-2026-004 ITEM 06: partial-payment allocation ledger (same schema as
    // current-year Record Payment) — omitted on full payments so legacy consumers are unaffected.
    // _syncStudentFinancials reads monthAllocations to mark PARTIAL + write per-month monthShortage.
    ...(_pdIsPartial ? { monthAllocations: _pdMonthAllocations, partialMonths: _pdPartialMonths, monthShortage: _pdMonthShortage } : {}),
    ...
    date: firebase.firestore.Timestamp.fromDate(
      (() => { const [yr,mo,dy] = date.split('-').map(Number); const n = nowIST(); /* ITEM 01 FIX: build the IST wall-clock instant, not device-local */
               return new Date(Date.UTC(yr, mo-1, dy, n.getHours(), n.getMinutes(), n.getSeconds(), n.getMilliseconds()) - 5.5*60*60*1000); })()
    ),
    receiptNumber,
...
function _pastDueLockForm(studentName, year, months) {
  ...
  const _btn = document.querySelector('[onclick="_pastDueSave()"]');
  if (_btn) {
...
function _pastDueShowReceipt(tx, student, year, months, amount, date) {
  ...
  document.getElementById('pdr_details').innerHTML = `
    ...
    <div class="r-row"><span class="r-lbl">Date &amp; Time</span><span class="r-val">${(()=>{const d=tx.date?.toDate?tx.date.toDate():(typeof date==='string'?new Date(date.includes('T')?date:date+'T00:00:00'):new Date(date));return d.toLocaleDateString('en-IN', {timeZone:IST_TZ,day:'2-digit',month:'short',year:'numeric'})+', '+d.toLocaleTimeString('en-IN', {timeZone:IST_TZ,hour:'2-digit',minute:'2-digit',hour12:true});})()}</span></div>
  `;
...
function _pastDuePrint() {
  ...
  const txData = {
    ...
    date:            new Date(date).toLocaleDateString('en-IN', {timeZone:IST_TZ, day:'2-digit', month:'short', year:'numeric' }),
  ...
  setContent(`
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div id="excusedForm" style="pointer-events:none;opacity:0.4">
            ...
            <!-- Step 2: Month grid -->
            <div class="form-group">
              <label class="form-label">Step 2 — Select Months to Excuse *
            ...
            <div class="form-group">
              <label class="form-label">Step 3 — Justification / Reason * <span style="color:var(--danger);font-size:11px">(Cannot be blank)</span></label>
    ...
    <div class="card" style="margin-top:20px">
      ...
      <div class="card-body" style="padding:0">
        <div class="tbl-wrap">
          <table>
            <thead><tr><th>Name</th><th>Adm#</th><th>Class</th><th>Months Excused</th><th>Last Waiver Date</th><th>Reason</th><th>Approved By</th><th>Remaining Balance</th></tr></thead>
            <tbody id="excusedCandBody"><tr><td colspan="8" style="text-align:center;padding:30px;color:var(--muted)">Loading…</td></tr></tbody>
          </table>
        </div>
      </div>
    </div>
...
async function _excusedRenderCandidates() {
  ...
  try {
    ...
    if (studentIds.length === 0) {
      body.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:30px;color:var(--muted)">No excused waivers recorded yet.</td></tr>`;
    ...
    body.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:30px;color:var(--danger)">Error loading excused candidates: ${sanitizeHTML(e.message)}</td></tr>`;
  }
}
...
async function _excusedPick(sid) {
  ...
  // SYNC FIX: Lock already-paid months — mirrors selectFeeStudent() logic exactly.
  // Fetches current-year feeTransactions + student monthStatus to build paidMonths set,
  // then disables those pills so staff cannot waive a month already paid or excused.
  try {
    ...
    const paidMonths    = new Set(); // fee payments
    const excusedMonths = new Set(); // excused waivers — shown differently (green, "✓ EXCUSED")
    ...
    txSnap.docs.forEach(doc => {
      ...
      if (d.type === 'excused_waiver' && Array.isArray(d.monthsExcused)) {
        d.monthsExcused.forEach(m => excusedMonths.add(m));
      } else {
        if (Array.isArray(d.monthsSelected)) d.monthsSelected.forEach(m => paidMonths.add(m));
        if (Array.isArray(d.monthsExcused))  d.monthsExcused.forEach(m => paidMonths.add(m));
      }
    });

    const studentYearNorm = _normaliseAcademicYear(s.academicYear || '');
    const _isCurrentYear  = !studentYearNorm || studentYearNorm === _curYrNorm;
    if (_isCurrentYear && s.monthStatus && typeof s.monthStatus === 'object') {
      Object.entries(s.monthStatus).forEach(([month, status]) => {
        if (status === 'N/A-PAID' || status === 'PAID') paidMonths.add(_shortToFull[month] || month);
      });
    }

    document.querySelectorAll('#excusedMonthGrid button').forEach(btn => {
      const m = btn.dataset.month;
      if (excusedMonths.has(m)) {
        // Previously excused — bright green, "✓ EXCUSED" label
        btn.dataset.paid     = 'true';
        btn.disabled         = true;
        btn.title            = `${m} already excused (waived) — cannot waive again`;
      ...
      } else if (paidMonths.has(m)) {
        // Fee paid — muted green, "✓ PAID" label
        btn.dataset.paid     = 'true';
        btn.disabled         = true;
        btn.title            = `${m} already paid — cannot waive`;
      ...
      if (!cSnap.empty) {
        ...
        if (activeMonths.length > 0) {
          ...
          const coveredMonthNames = new Set(
            activeMonths.map(k => { const[,mm] = k.split('-'); return _MONTH_NAME[mm]; }).filter(Boolean)
          );
          document.querySelectorAll('#excusedMonthGrid button').forEach(btn => {
  ...
  // Reset waiver amount display
  const _waiverEl = document.getElementById('excusedWaiverAmount');
  const _waiverHint = document.getElementById('excusedWaiverHint');
  if (_waiverEl) { _waiverEl.value = '0'; }
  if (_waiverHint) { _waiverHint.textContent = ''; }
...
function _excusedToggleMonth(btn) {
  ...
  // Calculate waiver amount: standard rate × months selected (full fee waived)
  const cls      = student ? (student.class || student.cls || '') : '';
  const stdRate  = cls ? (getClassRate(cls)?.rate || _FEE_SCHEDULE[cls] || 0) : 0;
...
async function _excusedSave() {
  ...
  const _excCls     = student.class || student.cls || '';
  const _excInfo    = getClassRate(_excCls);
  const _excStdRate = _excInfo?.rate || 0;
  const _excCoveredMonths = new Set(state.concessionActiveMonths || []);
  const _excConcRate = state.concessionRate;
  const _EXC_MONTH_NUM = {
    January:'01', February:'02', March:'03', April:'04',
    May:'05', June:'06', July:'07', August:'08',
    September:'09', October:'10', November:'11', December:'12'
  };
  const _excAcadYrStr = _getCurrentAcademicYearStr();
  const _excStartYr   = parseInt(_excAcadYrStr.split('-')[0], 10);
  const _excEndYr     = _excStartYr + 1;
  function _excRateForMonth(monthName) {
    if (typeof _excConcRate !== 'number' || _excConcRate < 0) return _excStdRate; // no concession
    const mm = _EXC_MONTH_NUM[monthName];
    if (!mm) return _excStdRate;
    const numericMM = parseInt(mm, 10);
    const yearForMonth = numericMM >= 6 ? _excStartYr : _excEndYr;
    const key = `${yearForMonth}-${mm}`;
    return _excCoveredMonths.has(key) ? _excConcRate : _excStdRate;
  }
  ...
  const tx = {
    ...
    feeHead:        `EXCUSED WAIVER — ${months.join(', ')}`,
  ...
  try {
    ...
    const newBalance = Math.max(0, totalOutstanding - deduct);
...
function _excusedShowReceipt(tx, student, months, date, reason, approvedBy) {
  ...
  document.getElementById('exr_details').innerHTML = `
    ...
    <div class="r-row"><span class="r-lbl">Date &amp; Time</span><span class="r-val">${(()=>{const d=new Date(date);return d.toLocaleDateString('en-IN', {timeZone:IST_TZ,day:'2-digit',month:'short',year:'numeric'})+', '+d.toLocaleTimeString('en-IN', {timeZone:IST_TZ,hour:'2-digit',minute:'2-digit',hour12:true});})()}</span></div>
  `;
...
function _excusedPrint() {
  ...
  const txData = {
    ...
    date:            new Date(d.date).toLocaleDateString('en-IN', {timeZone:IST_TZ,day:'2-digit',month:'short',year:'numeric'}),
    paymentMode:     'N/A — EXCUSED WAIVER',
    paymentStatus:   'EXCUSED',
