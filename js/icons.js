...
async function _pastDuePick(sid) {
  ...
  if (_pdCtxEl) {
    ...
    // _normYrBanner lived here — a local re-implementation of academic-year
    // normalisation, used only by the heuristic below it. The engine normalises
    // through _normaliseAcademicYear, so with the heuristic gone this was a
    // second definition of one rule with no callers. Removed rather than left
    // for someone to reach for.


    // ══════════════════════════════════════════════════════════════════════════
    // ONE ENGINE ANSWERS "WHAT DOES THIS STUDENT STILL OWE FOR YEAR X".
    //
    // This map was built from FOUR heuristic sources — the doc's academicYear
    // grid, previousYearMonthStatus, each openingOutstandingDues entry, and a
    // single-year onboarding fallback — then patched by a fifth "live
    // cross-check" pass that re-derived unpaid months from transactions when a
    // source produced zero. Every layer carried its own bug-fix comment
    // (BUG-SRC4-DOUBLECOUNT, BUG-SRC4-FIELDNAME, ITEM-9(c), BUG-ITEM9C-FALSEDUE),
    // which is what a heuristic looks like when it is repeatedly wrong.
    //
    // It was wrong again here. KABIR KAPOOR's 2025-26 is fully cleared — the
    // engine prices it at 0 and previewPriorYearCrossCheck agrees across all 152
    // students — yet this banner offered 2025-26: ₹1,700 for collection. Asking a
    // clerk to take money for a year that is already settled is the worst
    // direction for this particular screen to be wrong in.
    //
    // _flPriorYearBreakdown is the same engine every other surface uses, already
    // excludes the current year, and is what previewPriorYearCrossCheck compares
    // against — so this banner can no longer disagree with the check that audits
    // it. The shape is unchanged: { [year]: amount }, which _pastDueSave() reads
    // to compute a receipt's remaining balance.
    //
    // A failure here empties the banner rather than guessing. A blank banner is a
    // clerk asking a question; an invented one is a family paying twice.
    // ══════════════════════════════════════════════════════════════════════════
    const _bannerYearMap = {};
    let _allTxB = [];
    try {
      _allTxB = (await schoolCol('feeTransactions').where('studentId', '==', s.id).get()).docs.map(d => d.data());
    } catch (_) { _allTxB = []; }
    try {
      const _bd = _flPriorYearBreakdown(s, _allTxB, _getCurrentAcademicYearStr());
      (_bd.years || []).forEach(y => {
        if (Number(y.outstanding) > 0) _bannerYearMap[y.year] = Number(y.outstanding);
      });
    } catch (_bdErr) {
      console.error('[PAST DUE] prior-year breakdown failed for ' +
        (s.admissionNumber || s.id) + '. The banner is left empty rather than ' +
        'estimated — do not collect against it until this is resolved:',
        (_bdErr && _bdErr.message) || _bdErr);
    }
        ...
        <div style="padding:14px 16px;border-radius:10px;margin-bottom:14px;
          ...
          <div>
            ...
            <div style="font-size:12px;color:var(--muted);line-height:1.7">
              ...
              ${(typeof _flPaymentGuard === 'function' && !_flPaymentGuard(s).allowed)
                ? `<br><br><span style="color:var(--danger);font-weight:700">This student is TERMINATED</span> —
                   payments cannot be recorded for them. Collect arrears through
                   <strong>Pay Dues</strong> on the Terminated Students screen, or restore
                   them with Undo Terminate first.`
                : `<br><br>To record a fee payment for the current year, use
              <a href="#" onclick="pushNav('recordFee',{studentId:'${s.id}',studentName:'${jsAttr(s.name)}'});return false;"
...
async function _pastDueLoadMonthGrid(year, sid, openingMonths, studentData) {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // WHICH GRID DESCRIBES THE YEAR THE CLERK PICKED? ASK THE ONE FUNCTION.
  //
  // Source 2 chose between monthStatus and previousYearMonthStatus on a single
  // test: "is the selected year the student's current year?" If not, it read
  // previousYearMonthStatus — for ANY earlier year. That field describes exactly
  // one year, the one named by previousAcademicYear, so on a student with three
  // years on record the wrong grid answered.
  //
  // Live: KABIR KAPOOR. Selecting 2024-25 read his 2025-26 grid, where September
  // is N/A-PAID, so September 2024-25 locked green — and September 2024-25 is
  // the ONE month he still owes for that year. The ₹1,700 the banner correctly
  // reported was uncollectable, because the only month that could clear it was
  // displayed as already settled.
  //
  // Source 3 then read openingOutstandingDues correctly, but added to the SAME
  // paid set and never removed what Source 2 had wrongly put there — so the
  // right answer could not overturn the wrong one.
  //
  // _flGridForYear is the resolver the engine bills through. It knows
  // openingOutstandingDues[] wins, then academicYear→monthStatus, then
  // previousAcademicYear→previousYearMonthStatus, then
  // openingOutstandingYear→prevYearMonthStatus — and it returns {} rather than
  // guessing when a year has no grid at all.
  //
  // EXCUSED is handled here too, and was not before: it is neither N/A-PAID nor
  // DUE, so a waived month fell through every branch below to "not flagged, still
  // selectable" and was offered for collection in red. Money the school had
  // already forgiven, presented as payable.
  // ══════════════════════════════════════════════════════════════════════════
  const excused = new Set();
  const _yearGrid = (typeof _flGridForYear === 'function') ? _flGridForYear(sData, year) : {};
  let _gridOwedMonths = null;
  if (_yearGrid && Object.keys(_yearGrid).length) {
    const _due = [];
    Object.entries(_yearGrid).forEach(([month, status]) => {
      const fullName = _pdShortToFull[month] || month;
      const st = String(status || '').toUpperCase();
      if (st === 'N/A-PAID' || st === 'PAID') paid.add(fullName);
      else if (st === 'EXCUSED')              excused.add(fullName);
      else if (st === 'DUE')                  _due.push(fullName);
    });
    _gridOwedMonths = _due;
  } else {
    // No grid for this year. The openingOutstandingDues entry may still carry a
    // plain unpaidMonths list from a manual onboarding that never built a grid.
    const _arr = Array.isArray(sData.openingOutstandingDues) ? sData.openingOutstandingDues : [];
    const _hit = _arr.find(e => e && _normaliseAcademicYear(e.year || '') === _normYear);
    if (_hit && Array.isArray(_hit.unpaidMonths)) _gridOwedMonths = _hit.unpaidMonths;
  }

  // Per-year owed months from the resolved grid; otherwise the passed openingMonths.
  const owedSet = new Set(_gridOwedMonths !== null ? _gridOwedMonths : openingMonths);
  ...
  document.querySelectorAll('#pastDueMonthGrid .month-pill').forEach(p => {
    ...
    } else if (excused.has(m)) {
      // EXCUSED — waived, nothing is owed. Locked like a paid month but labelled
      // for what it is: this branch did not exist, so a waived month fell through
      // to "not flagged, still selectable" and was offered for collection in red.
      p.dataset.paid     = 'true';        // same contract as paid: not payable
      p.disabled         = true;
      p.style.background = 'rgba(74,158,202,0.15)';
      p.style.color      = 'var(--info)';
      p.style.borderColor= 'rgba(74,158,202,0.35)';
      p.title            = m + ' — EXCUSED (fee waived, nothing to collect)';
      p.innerHTML        = `${m.slice(0,3)}<br><span style="font-size:8px;font-weight:700;letter-spacing:0.3px">EXCUSED</span>`;
    } else if (paid.has(m)) {
...
async function _pastDueSaveInner() {
  ...
  const _ACAD_PD = ACAD_MONTHS_FULL;
  ...
  try {
    ...
    try {
      ...
      const _studentUpdatePayload = {
        ...
        // S2: remainingBalance on the STUDENT doc is gone — it duplicated
        // outstandingBalance, nothing read it, and only some writers maintained
        // it. _syncStudentFinancials removes the stored field on reconcile. The
        // remainingBalance written onto the TRANSACTION above is unaffected: that
        // one is the receipt's frozen balance and is read all over the app.
...
async function _excusedSave() {
  ...
  const tx = {
    ...
    // This used to cite selectFeeStudent()'s prevBalance lookup as the reason; that
    // query is gone, but the requirement is not. Five orderBy('date','desc') queries
    // still run against feeTransactions (Dashboard, Finance, Hidden, Search) and
    // sixteen call sites sort on `date.seconds` directly. A string date sorts
    // lexically at best and is absent from an orderBy index at worst.
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      transaction.update(stuRef, {
        ...
        // S3: fee_status is NOT written here any more. This two-state rule could
        // only say paid or pending, so waiving a month on a student who held an
        // open partial elsewhere silently demoted them out of 'partial' — the one
        // state filters.js offers as a filter and the one only the engine emits.
        // _syncStudentFinancials, called immediately after this transaction,
        // computes it across every year through _flFeeStatusFor.
        updatedAt:          firebase.firestore.FieldValue.serverTimestamp(),
      });
    });

    // Audit log
    // auditLog cannot throw — see search.js. This sits between the waiver
    // committing and the receipt rendering, which is exactly why that guarantee
    // has to live in the function rather than in a wrapper here.
    auditLog('excused_waiver', { studentName: student.name, months, reason, approvedBy, receiptNumber, newOutstandingBalance: newBalance });
