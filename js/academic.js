...
function _acApplyFilters() {
  ...
  tbody.innerHTML = base.map(s => {
    ...
    return `<tr>
      ...
      <td><strong style="cursor:pointer;color:var(--text)" onclick="renderStudentProfile('${s.id}',${JSON.stringify(s).replace(/"/g,'&quot;')})">${s.name||'—'}</strong></td>
      ...
      <td>
        <button class="btn btn-ghost btn-sm" onclick="renderStudentProfile('${s.id}')">View</button>
