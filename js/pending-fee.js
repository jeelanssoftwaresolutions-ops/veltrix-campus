    ...
    const classify = last => {
      ...
      const when  = last == null ? 'never run on this browser'
                  ...
                  : days === 1  ? 'run yesterday'
                                : `run ${days} days ago`;
      return { days, stale, when };
    };

    const never   = classify(null);
    const today   = classify(Date.now());
    const yest    = classify(Date.now() - DAY);
    const day29   = classify(Date.now() - 29 * DAY);
    const day30   = classify(Date.now() - 30 * DAY);
    const ancient = classify(Date.now() - 400 * DAY);
...
function _flPaymentGuard(sData, opts) {
  ...
  if (status !== 'terminated') return { allowed: true, status };
  ...
  return {
    allowed: false,
    status,
    reason: 'This student is TERMINATED. Payments cannot be recorded from here. ' +
...
async function previewHealthCheck() {
  ...
  await run('aggregate vs engine (all years)', typeof previewSyncCrossCheck === 'function' ? previewSyncCrossCheck : null,
    r => ({ ok: r && r.rows.length === 0,
            text: r ? `${r.rows.length} out of sync of ${r.scanned}` : 'no result' }));
  ...
  try { localStorage.setItem(_FL_HEALTH_KEY, JSON.stringify({ at: Date.now() })); } catch (_) {}
