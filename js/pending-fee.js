...
function _flAppliedByMonthFromTxs(yearTxs, rateForMonth) {
  ...
  (yearTxs || []).forEach((t, _i) => {
    ...
    if (t.monthAllocations && typeof t.monthAllocations === 'object') {
      ...
      // real debt: Naveen Joshi (Grade 6 @ 1,800, no concession) paid 3,600
...
// is what manufactured Dinesh Bhat's phantom dues.
//
...
async function _flRunContractTests() {
  ...
  if (_FL_CONC_BY_ADM === null) { _FL_CONC_BY_ADM = {}; _FL_CONC_OK = true; }
  ...
  T('student_ref_resolves_to_real_student_id_not_foreign_doc_id', () => {
    const students = [
      { id: 'stu_ravi',  admissionNumber: 'ADM-2025-051', name: 'Ravi Nahdi' },
      { id: 'stu_aisha', admissionNumber: 'ADM-2026-008', name: 'Aisha Khan' },
    ];
    const r = (ref) => _flResolveStudentRefId(ref, students);
    const txRow      = r({ id: 'RCP_txdoc_9f2', studentId: 'stu_ravi', admissionNumber: 'ADM-2025-051', studentName: 'Ravi Nahdi' });
    const concRow    = r({ id: 'conc_doc_77',  admissionNo: 'ADM-2026-008' });        // no studentId at all
    const legacyRow  = r({ id: 'legacy_doc_5', admissionNumber: 'ADM-2025-051' });    // foreign id + adm
    const termRow    = r({ id: 'term_doc_3',   studentId: 'stu_aisha' });
    const studentRow = r({ id: 'stu_aisha', admissionNumber: 'ADM-2026-008' });       // /students row
    const dueFeeRow  = r({ studentId: 'stu_ravi', name: 'Ravi Nahdi' });              // Due Fee shape
  ...
  T('total_collected_identical_across_dashboard_bar_and_buckets', () => {
    const txs = [
      ...
      { academicYear:'2026-27', amountPaid:3400,  paymentMode:'Split Payment',       // the Ravi Nahdi shape
  ...
  //     engine performs. Dinesh Bhat's real document: academicYear 2026-27, a fully-paid
  ...
  //     Ashok Kumar's document must be unaffected: his May really is DUE, so 1,700 must stand.
  ...
  // 40) ITEMS 1/16 PART 2 — Inaaya Khan's REAL document (ADM-2026-002). academicYear
  ...
  T('dryrun_report_totals_reconcile_and_pin_named_students', () => {
    ...
    const dinesh = mk('Dinesh Bhat', 'ADM-2025-071', 11900, 0, { noGridYears: ['2026-27'] });
    const inaaya = mk('Inaaya Khan', 'ADM-2026-002', 8500, 0,  { noGridYears: ['2026-27'] });
    const ashok  = mk('Ashok Kumar', 'ADM-2025-092', 1700, 1700);           // unchanged on purpose
    const promoted = mk('Promoted Kid', 'ADM-2026-050', 0, 300, {
    ...
    const pinned = _FL_PINNED_ADMISSIONS.map(adm => rows.find(r => r.admissionNumber === adm));
    ...
    const html = _flBuildDryRunReportHTML({
      ...
      noGrid: [{ name:'Inaaya Khan', admissionNumber:'ADM-2026-002', class:'LKG', years:'2026-27' }],
    ...
    return { pass: upTotal === 300 && downTotal === -20400          // the two directions, separately
                 ...
                 && html.indexOf('Ashok Kumar') > -1               // pinned though unchanged
             ...
             detail: { up: upTotal, down: downTotal, net: netDelta,
                       ...
                       ashokPinnedAndUnchanged: html.indexOf('Ashok Kumar') > -1 && !ashok.changed } };
  });
  ...
  //     empty now that ADM-2026-002 has been released, and a test pinned to one
  ...
  T('quarantine_skips_only_the_named_student', () => {
    ...
    const other  = _flIsQuarantined('normalId', 'ADM-2025-071', { skipAdmissionNumbers:[ADM] });
  ...
  T('due_fee_uses_the_engine_for_every_student_part_b_shipped', () => {
    ...
    //     months. This is the branch that manufactured Dinesh Bhat's phantom 11,900.
  ...
  // 56) INAAYA'S ROOT — an implausible class jump on import must be refused, and two
  ...
  T('inaaya_root_class_jump_guard_and_grid_conflict_detection', () => {
    ...
    const s = { admissionNumber:'ADM-2026-002', academicYear:'2025-26',
  ...
  T('l7_profile_rate_matches_engine_rate', () => {
    ...
    const inaaya = { class:'LKG', monthlyFee:1500, admissionNumber:'ADM-2026-002' };
  ...
  //     must not fall out of the promotion chain. Found live: ADM-2026-002 stored as
  ...
  T('item116p2_current_year_without_grid_still_bills_the_full_year', () => {
    ...
    // A plain current-year student who has simply paid nothing (ADM-2026-011 et al).
  ...
  //     Naveen Joshi's real receipts, and the item 14 contract that must survive.
...
//   Suraj Gupta   ADM-2025-115  Grade 6  stored 18,700 = 11 x 1,700
//   Bhanu Joshi   ADM-2025-083  Grade 6  stored 20,400 = 12 x 1,700
...
//
// FOUND IN LIVE DATA. ADM-2026-002 carries class "LKG " — with a trailing space.
...
//   Ravi Verma   Grade 4 @ 1700, concession 775 on Jun-Nov 2026.
...
//   Arjun Mishra Grade 1 @ 1700, concession 1500 on Dec 2026-May 2027.
...
function _flConcessionFor(s) {
  if (!s) return null;
  if (_FL_CONC_BY_ADM === null || !_FL_CONC_OK) {
    if (!_FL_CONC_WARNED) {
      _FL_CONC_WARNED = true;
      console.warn('[CONCESSION] The fee engine priced a student while the concession ' +
        'register was ' + (_FL_CONC_BY_ADM === null ? 'not yet loaded' : 'in a FAILED state') +
...
//
// ADM-2026-002 is the live case: openingOutstandingDues[0] says 2025-26 is fully paid
...
function _flStudentYearOutstanding(s, yearTxs, yr, opts) {
  ...
  // them (ADM-2026-011/012/014/015) are not even carry-overs; they are plain
  ...
  // twelve months there is what manufactured Dinesh Bhat's phantom 11,900. (His case
  ...
  try {
    ...
    if (Array.isArray(s.openingOutstandingDues) && s.openingOutstandingDues.length) {
      const newArr = s.openingOutstandingDues.map(entry => {
        ...
        //
        // UMAR BIN AL NASAR (ADM-2026-006, active) had the two years crossed: 2025-26
...
// Students the reviewer asked to see regardless of where they sort. These three are
// the live evidence this whole engine change was diagnosed from, so they are pinned
// to the top of the report for cross-checking against the running app.
const _FL_PINNED_ADMISSIONS = ['ADM-2025-071', 'ADM-2026-002', 'ADM-2025-092'];
async function previewReconcileAllStudentDues(opts) {
  ...
  const pinned = _FL_PINNED_ADMISSIONS
...
// settled, so the discount can never apply to anything. Inaaya Khan is one:
...
//
// ADM-2026-002 (Inaaya Khan): reported showing three different classes across three
...
// ADM-2026-002 / wFrUNYvgBtYRZxWlvZ9a — RELEASED. Her three-classes-at-once symptom
...
async function renderPendingFee() {
  ...
  try {
    ...
    Object.values(stuMap).forEach(s => {
      ...
      //   · WAIVERS — Bhanu Singh's 800 September remainder, excused and ignored.
      //   · CASH-CAPPED CREDIT — Naveen Joshi 17,000 → 8,000, the one moving the other
...
function _pfDueCells(r, forPdf) {
  return [
    ...
    forPdf ? '₹' + fmtNum(r.amountDue || 0) : Number(r.amountDue || 0),
...
async function _pfDueExportPDF(rows, meta) {
  ...
  doc.text(`Students: ${list.length}  |  Total outstanding: ₹${fmtNum(total)}`, 14, 27);
  doc.autoTable({
    startY: 31,
    head: [_PF_DUE_HEAD],
    body: list.map(r => _pfDueCells(r, true)),
    foot: [['', '', '', '', '', '', 'TOTAL', '₹' + fmtNum(total), '', '']],
