...
// ════════════════════════════════════════════════════════════════════════════
// THIS MUST NEVER THROW AT ITS CALLER, AND MUST NEVER FAIL QUIETLY.
//
// Two things were true at once. The promise rejection was already caught, so an
// audit write that Firestore refused was logged — as a warn, worded "non-fatal",
// alongside nothing identifying WHICH action went unrecorded. And the body
// itself could still throw SYNCHRONOUSLY (schoolCol, currentProfile, a missing
// tenant), which is a different failure entirely.
//
// That mattered because of where this is called from. Eighteen call sites,
// almost all of them immediately after money has moved: a payment, a waiver, a
// termination, a concession change. record-payment.js calls it between the
// transaction committing and the receipt being shown. A synchronous throw there
// would abort the receipt for a payment that had already been taken.
//
// Three call sites had noticed and wrapped it in try/catch; the other fifteen
// had not. Rather than add fourteen more wrappers, the guarantee belongs here:
// this function now cannot throw, so every caller is safe and the three local
// wrappers are redundant.
//
// The failure log is an error, not a warn, and it names the action. An audit
// trail is a compliance artefact — "some audit write failed at some point" is
// not something anyone can act on, whereas "excused_waiver was not recorded" is.
// Deliberately NOT surfaced to the clerk: interrupting a completed payment with
// a dialog about logging would train people to dismiss it.
// ════════════════════════════════════════════════════════════════════════════
function auditLog(action, details={}) {
  try {
    ...
    schoolCol('auditLogs').add({
      ...
      details
    }).catch(err => console.error('[AUDIT] "' + action + '" was NOT recorded in the audit ' +
      'trail. The action itself succeeded; only the log entry failed. Reason: ' +
      ((err && err.message) || 'unknown'), { action, details }));
  } catch (e) {
    console.error('[AUDIT] "' + action + '" could not be submitted to the audit trail at all. ' +
      'The action itself is unaffected. Reason: ' + ((e && e.message) || 'unknown'),
      { action, details });
  }
}
// ─────────────────────────────────────────────────────────────────────────────
...
async function doSearch(q) {
  try {
    ...
    // A non-principal never sees a departed student here. This dropdown was the
    // route into a hidden student's profile for an Admin — it listed them, and
    // the meta line below called them "Terminated", so nothing on screen even
    // suggested a confidential record had been surfaced.
    const _seeDeparted = (typeof _flMaySeeDeparted === 'function')
      ? _flMaySeeDeparted() : (currentRole === 'principal');
    const results = students.filter(s =>
      (_seeDeparted || !_flIsDeparted(s)) && (
    ...
    // The meta line below used to read status==='active' ? 'Active' : 'Terminated',
    // which labelled a HIDDEN student "Terminated" — wrong in itself, and it
    // disguised the leak. Only a principal reaches a departed student here at
    // all now, and when they do the line says which state it actually is.
    ...
    html += results.map(s=>`
      <div class="s-item" onmousedown="selectSearchResult('${s.id}')">
        ...
        <div class="s-meta">Adm# ${s.admissionNumber||'—'} · ${s.class||''} ${s.section||''} · ${
          s.status === 'hidden' ? 'Hidden · Confidential'
