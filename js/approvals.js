...
async function handleApproval(reqId, recordType, recordId, approve) {
  try {
    ...
    if (approve && recordType === 'student') {
      ...
      // BUG-N08 FIX: Compute financials from transactions (same logic as terminateStudent())
      ...
      <div class="card-body">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:12px" id="feeStructGrid">
          ${(() => {
            ...
            return Object.entries(getFeeSchedule())
              .sort(([a], [b]) => (classOrder[a] ?? 99) - (classOrder[b] ?? 99))
              .map(([cls, rate]) => `
            <div style="background:rgba(14,28,18,0.40);border:1px solid var(--glass-border);border-radius:10px;padding:14px 16px">
              ...
              <div class="fee-struct-display" data-class="${cls}" style="font-size:22px;font-weight:700;color:var(--gold-lt);font-family:'Bebas Neue',sans-serif;letter-spacing:1px">₹${fmtNum(rate)}</div>
