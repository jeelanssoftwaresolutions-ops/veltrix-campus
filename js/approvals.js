        ...
        if (studentId) {
          try {
            ...
            await schoolCol('students').doc(studentId).update({
              // S3: fee_status is NOT written here any more. This wrote a flat
              // 'pending' and its own comment called it provisional — but a
              // student who legitimately holds an open partial was demoted to
              // 'pending' by it, and only _syncStudentFinancials (called below)
              // can produce 'partial'. Writing a value that is both lossy and
              // overwritten milliseconds later cost information and bought none.
