...
/** Toggle terminated shim for legacy call sites */
function toggleTerminatedStudents() { _smToggleTerminated(); }
...
function _renderStudentRows(list) {
  ...
  list.forEach(s => {
    ...
    rows.push(`<tr data-status="${s.status||'active'}">
      <td>${blockBadge}</td>
      ...
      <td>${s.parentName||'—'}</td>
      <td>${s.contact||'—'}</td>
      <td>${isActive?'<span class="badge badge-green">Active</span>':'<span class="badge badge-red">Terminated</span>'}</td>
      <td>
...
/* ============================================================
   STUDENT PROFILE
   ============================================================ */
async function renderStudentProfile(id, preloaded) {
  ...
  async function _doRenderProfile(studentData, txDocs, concessionData) {
    ...
    const curYearTx   = transactions.filter(t => {
      const ty = (t.academicYear || t.feeYear || '');
      return !ty || _normaliseAcademicYear(ty) === _normaliseAcademicYear(curAcadYear);
    });
    ...
    setContent(`
      <div class="profile-hero">
        ...
        <div style="flex:1">
          ...
          <div class="profile-badges">
            <span class="badge ${s.status==='active'?'badge-green':'badge-red'}">${s.status==='active'?'Active':'Terminated'}</span>
            
          </div>
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          <button class="btn btn-secondary btn-sm" onclick="showEditStudentModal('${s.id}')">Edit Profile</button>
          ${currentRole==='principal'&&s.status==='active'?`<button class="btn btn-danger btn-sm" onclick="terminateStudent('${s.id}','${(s.name||'').replace(/'/g,"\\'")}')">Terminate Student</button>`:''}
          ${currentRole==='principal'&&s.status==='active'?`<button class="btn btn-ghost btn-sm" style="border-color:rgba(212,150,42,0.4);color:var(--warn)" onclick="moveStudentToHidden('${s.id}','${(s.name||'').replace(/'/g,"\\'")}')">🔒 Move to Hidden</button>`:''}
          ${currentRole==='principal'?`<button class="btn btn-sm" style="background:rgba(224,82,82,0.08);color:var(--danger);border:1px solid rgba(224,82,82,0.3);font-size:12px" onclick="deleteStudentPermanent('${s.id}','${(s.admissionNumber||'').replace(/'/g,"\\'")}','${(s.name||'').replace(/'/g,"\\'")}')">🗑 Delete Student</button>`:''}
          ...
          <button class="btn btn-primary btn-sm" onclick="pushNav('recordFee',{studentId:'${s.id}',studentName:'${(s.name||'').replace(/'/g,"\\'")}',classSection:'${(s.class||'')+'  –  Section '+(s.section||'')}' })">Record Payment</button>
      ...
      ${(()=>{
        // ── CONSTANTS ──
        const ACAD_MONTHS      = ['Jun','Jul','Aug','Sep','Oct','Nov','Dec','Jan','Feb','Mar','Apr','May'];
        const ACAD_MONTHS_FULL = ['June','July','August','September','October','November','December','January','February','March','April','May'];

        // ── MONTHLY FEE from student doc or fee schedule ──
        const monthlyFee = s.monthlyFee || (typeof _FEE_SCHEDULE !== 'undefined' ? (_FEE_SCHEDULE[s.class] || 0) : 0);
        ...
        const _profileRateForMonth = (shortMonth) => {
          if (typeof _concFee !== 'number') return monthlyFee;
          if (_concActiveMonths.length === 0) return _concFee; // indefinite concession
          const _MN = {Jun:'06',Jul:'07',Aug:'08',Sep:'09',Oct:'10',Nov:'11',Dec:'12',Jan:'01',Feb:'02',Mar:'03',Apr:'04',May:'05'};
          const mm = _MN[shortMonth];
          if (!mm) return monthlyFee;
          return _concActiveMonths.some(k => k.endsWith('-'+mm)) ? _concFee : monthlyFee;
        };
        ...
        const outstandingBal = _curYrTxWithBal.length > 0
          ? (_curYrTxWithBal[0].remainingBalance || 0)
          : _pillBasedDue;
        ...
        const buildMonthGrid = (statusObj) => {
          return ACAD_MONTHS.map(short => {
            ...
            const cls    = isPaid ? 'fms-paid' : 'fms-due';
            const label  = isPaid ? 'PAID' : 'DUE';
            const bg     = isPaid ? 'rgba(201,168,76,0.10)' : 'rgba(224,82,82,0.07)';
        ...
        if (Array.isArray(s.openingOutstandingDues)) {
          s.openingOutstandingDues.forEach(d => {
            ...
            prevEntries.push({
              ...
              monthsCleared: Object.values(_dGrid).filter(v => (v||'').toUpperCase()==='N/A-PAID').length,
            });
          });
        }
        ...
        // AND a real unpaid balance sitting in studentDocYear (e.g. Ashish Patel: paid
        ...
        if (prevAcadYear && prevAcadYear !== curAcadYear && prevAcadYear !== studentDocYear && ((s.previousDues||0) > 0 || _prevHasTx)
          ...
          prevEntries.push({
            ...
            monthsCleared: Object.values(s.previousYearMonthStatus||s.prevYearMonthStatus||{}).filter(v=>(v||'').toUpperCase()==='N/A-PAID').length,
          });
        }
          ...
          if (isCurrent) {
            ...
            info.txList.forEach(t => {
              ...
              if (sel && Array.isArray(sel)) sel.forEach(m => {
                if (t.type === 'excused_waiver') excusedSet.add(m);
                else paidSet.add(m);
              });
            });
            // ITEM 13 FIX: "Onboard Existing Student" paid-at-entry months
            // (currentYearPaidMonths) — see matching fix on _paidOrExcusedThisYear
            // above for full context. Without this, months marked already-paid
            // during onboarding (no feeTransaction exists for them) render as DUE
            // in this card's own month grid even though the stat cards above it
            // were just corrected to exclude them.
            if (Array.isArray(s.currentYearPaidMonths) &&
                _normaliseAcademicYear(s.currentYearDueYear) === curAcadYear) {
              s.currentYearPaidMonths.forEach(m => paidSet.add(m));
            }
            // Also honour s.monthStatus for current year (if student was imported this year)
            // FIX (2026-07-07): use the same relaxed _mStatusIsCurrentYear check computed
            // above, so this grid stays in sync with the outstanding-balance calculation
            // and doesn't drop genuinely already-paid Excel-import months.
            const curMonthStatusObj = _mStatusIsCurrentYear ? (s.monthStatus||{}) : {};

            // JSS-REF-VELTRIX-2026-004 ITEM 06: per-month PARTIAL balance-left — latest
            // tx.monthShortage per month across this year's transactions.
            const _txMsPC = t => (t.date?.toMillis ? t.date.toMillis() : (t.date?.seconds ? t.date.seconds*1000 : new Date(t.date||0).getTime())) || 0;
            const shortByMonthPC = {};
            info.txList.filter(t => t.monthShortage && typeof t.monthShortage === 'object')
               .sort((a,b) => _txMsPC(a) - _txMsPC(b))
               .forEach(t => { Object.entries(t.monthShortage).forEach(([m, sh]) => { shortByMonthPC[m] = sh; }); });

            const curMonthCells = ACAD_MONTHS.map((short, i) => {
              const full = ACAD_MONTHS_FULL[i];
              // Priority: excused > paid (from tx) > monthStatus from doc > due
              let status = 'due';
              if (excusedSet.has(full)||excusedSet.has(short)) status = 'excused';
              else if (paidSet.has(full)||paidSet.has(short)) status = 'paid';
              else {
                const raw = (curMonthStatusObj[short]||'').toUpperCase();
                if (raw==='N/A-PAID'||raw==='PAID') status = 'paid';
                else if (raw==='PARTIAL') status = 'partial'; // JSS-REF-VELTRIX-2026-004 ITEM 06
              }
              ...
              const _isConcMonth = status === 'due' && _profileRateForMonth(short) !== monthlyFee;
              const _concLabel   = _isConcMonth
                ? `<div style="font-size:8px;color:var(--gold-lt);margin-top:3px;font-weight:700;letter-spacing:0.3px">CONCESSION</div>`
                : '';
              const _concTitle = _isConcMonth
                ? ` title="${short} — Concession rate ₹${fmtNum(_profileRateForMonth(short))}/month"`
                : '';
              return `<div${_concTitle} style="text-align:center;padding:8px 4px;border-radius:8px;border:1px solid ${_isConcMonth?'rgba(201,168,76,0.35)':'var(--glass-border)'};background:${_isConcMonth?'rgba(201,168,76,0.07)':bg}">
                <div style="font-size:10px;font-weight:700;color:var(--muted);margin-bottom:4px">${short}</div>
                <span class="fee-month-status ${cls}" style="font-size:9px;padding:2px 5px">${label}</span>
                ${_shPC}${_concLabel}
              </div>`;
            ...
            expandedHtml = `
              <div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--glass-border)">
                <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">
                  ...
                  ${tile('Annual Fee','₹'+fmtNum(monthlyFee*12),'₹'+fmtNum(monthlyFee)+' × 12 months','rgba(74,158,202,0.10)','rgba(74,158,202,0.25)','var(--info)')}
            ...
            // ── PREVIOUS YEAR ──
            // monthStatus source: info._monthStatus (injected above from correct Firestore field)
            // ITEM-9(b) FIX: The stored snapshot only reflects the state at import time and
            // (for legacy data written before this fix) may not reflect payments made later
            // via "Record Previous Year Dues". Merge in the actual paid months from this
            // year's live transaction log so the grid can never show a genuinely-paid month
            // as due, regardless of how stale the underlying snapshot is.
            const _fullToShortProf = {June:'Jun',July:'Jul',August:'Aug',September:'Sep',October:'Oct',November:'Nov',December:'Dec',January:'Jan',February:'Feb',March:'Mar',April:'Apr',May:'May'};
            const prevStatus  = { ...(info._monthStatus || {}) };
            info.txList.forEach(t => {
              if (t.type === 'excused_waiver') return; // excused months tracked separately, not "paid"
              (t.monthsSelected || []).forEach(m => {
                const shortKey = _fullToShortProf[m] || m;
            ...
            // MONTHCOUNT-FIX (2026-07-07): when the month-wise grid is actually available,
            // it is the source of truth the user sees on screen — count directly from
            // prevStatus (which already includes any later payments merged in above).
            // The stale info._monthsDue / info._monthsCleared counters are captured once at
            // import time and can drift out of sync with the grid after later payments are
            // recorded, which was making "Months Cleared" disagree with the visible pills.
            // Only fall back to those stale counters when there's no grid at all to count from.
            const mCleared    = hasGrid
                                ? ACAD_MONTHS.filter(m => {
                                    const raw = (prevStatus[m]||'').toUpperCase();
                                    return raw === 'N/A-PAID' || raw === 'PAID';
                                  }).length
                                : (info._monthsCleared != null ? info._monthsCleared : 0);
            // Due = whatever isn't cleared, over the full 12-month year — this mirrors
            // buildMonthGrid's own default (any month without an explicit PAID entry
            // renders as DUE), so the tile total always agrees with the pills below it.
            const mDue        = hasGrid ? (12 - mCleared)
                                : (info._monthsDue != null ? info._monthsDue : 0);

            // BUG-PROFILECARD-STALEBALANCE FIX: for a previous year that already has at
            // least one real transaction, the code above (JSS-BUG-FEECARD-001) trusts the
            // transaction's stored remainingBalance rather than re-deriving live — correct
            // in general, but that stored figure can itself have been wrong at write time
            // (e.g. computed against a since-corrected combined-balance bug) and never gets
            // corrected afterwards. The month-wise grid above (mCleared/mDue) is always
            // freshly derived from the actual paid/due status the user sees on screen, so
            // whenever the grid confidently shows every month cleared, trust that over a
            // possibly-stale stored balance rather than showing a due that contradicts the
            // 12/12 grid right below it (and reassign `outstanding` itself so the collapsed
            // card-shell header agrees with the expanded tiles too).
            if (hasGrid && mDue === 0) outstanding = 0;
            ...
            expandedHtml = `
              <div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--glass-border)">
                <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">
                  ...
                  ${tile('Annual Fee','₹'+fmtNum(prevFee*12),'₹'+fmtNum(prevFee)+' × 12','rgba(74,158,202,0.07)','rgba(74,158,202,0.20)','var(--info)')}
