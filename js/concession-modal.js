      ...
      drop.innerHTML = hits.length
        ? hits.map(s => {
            ...
            return `<div onclick="_acmPick('${s.id}')" style="padding:10px 14px;cursor:pointer;border-bottom:1px solid var(--glass-border-lt);display:flex;gap:10px;align-items:center;transition:background .12s"
              ...
              <div><div style="font-size:13px;font-weight:600;color:var(--text)">${sanitizeHTML(s.name||'—')}</div>
...
async function _acmPick(sid) {
  ...
  // Block terminated/hidden students — concession only applies to active students
  if (s.status && s.status !== 'active') {
    _acmAlert(`⚠️ ${s.name || 'This student'} is ${s.status} and cannot be given a concession. Only active students are eligible.`, 'danger');
    document.getElementById('_acmDrop').style.display = 'none';
    return;
  }
...
async function _acmDoSave() {
  ...
  // Double-check student is still active
  try {
    const sDoc = await schoolCol('students').doc(sel.id).get();
    const sData = sDoc.data();
    if (sData && sData.status && sData.status !== 'active') {
      _acmAlert(`⚠️ ${sel.name} is ${sData.status}. Concessions can only be set for active students.`, 'danger');
      return;
    }
  ...
  if (dateVal) {
    ...
    payload.effectiveFrom = isNaN(d.getTime())
  ...
  try {
    ...
    try { auditLog('concession_set', { studentName:sel.name, admissionNo:sel.admNo, concessionFee:operativeFee, reason }); } catch(_) {}
