...
async function _acmDoSave() {
  ...
  try {
    if (sel.existDocId) {
      // ══════════════════════════════════════════════════════════════════════
      // AN EDIT SUPERSEDES THE OLD CONCESSION. IT DOES NOT ERASE IT.
      //
      // A student holds ONE concession document, and this update replaces
      // activeMonths wholesale. So granting a second concession silently revoked
      // the first — retroactively, for months already closed under it.
      //
      // KARAN KAPOOR: July and August were paid in full at a 1,500 concession.
      // A later concession on November and December overwrote activeMonths, both
      // months reverted to the 1,700 class rate, and the 1,500 already collected
      // became 200 owing on each. 400 of debt the school never charged — and with
      // the old activeMonths gone there was nothing left to prove otherwise.
      //
      // The outgoing state is kept here so that question stays answerable: what
      // did this month cost on the day it was paid. _flRecordedMonthRates reads
      // it for payments older than tx.monthRates, and it is the audit trail for
      // every concession change besides.
      //
      // Read-modify-write rather than arrayUnion: a serverTimestamp sentinel is
      // not allowed inside an array element, so supersededAt is a plain ISO
      // instant. Capped so a heavily-edited concession cannot grow without bound.
      // ══════════════════════════════════════════════════════════════════════
      try {
        const _curSnap = await schoolCol('concessionFees').doc(sel.existDocId).get();
        const _cur     = _curSnap.exists ? _curSnap.data() : null;
        if (_cur) {
          const _prior = Array.isArray(_cur.history) ? _cur.history.slice(-19) : [];
          _prior.push({
            concessionFee:    (typeof _cur.concessionFee === 'number') ? _cur.concessionFee : null,
            standardFee:      (typeof _cur.standardFee   === 'number') ? _cur.standardFee   : null,
            activeMonths:     Array.isArray(_cur.activeMonths) ? _cur.activeMonths : [],
            monthlyBreakdown: (_cur.monthlyBreakdown && typeof _cur.monthlyBreakdown === 'object')
                                ? _cur.monthlyBreakdown : null,
            reason:           _cur.reason || '',
            supersededAt:     new Date().toISOString(),
            supersededBy:     setByName || ''
          });
          payload.history = _prior;
        }
      } catch (e) {
        // Not fatal to the grant itself, but it IS the evidence trail — say so
        // rather than letting a silent failure look like a clean edit.
        console.warn('[CONCESSION] Could not record the superseded concession for ' +
          (sel.admNo || sel.name || 'this student') + '. The new rate will be saved, but ' +
          'months closed under the OLD one have lost their proof of price:', e && e.message);
      }
