/* ============================================================
   ...
   ============================================================ */
function showApp() {
  // The school-hours gate that stood here is gone — see the block above
  // loadSchoolConfig() in core.js. It called isWithinSchoolHours(), which had
  // been returning a hardcoded true, so it never blocked anything.
...
function _touchSessionExpiry() {
  // ══════════════════════════════════════════════════════════════════════════
  // A FIXED EIGHT HOURS. NO SCHOOL-HOURS BRANCH, ON ANY CLOCK.
  //
  // This used to prefer a close-of-day instant built from the tenant's
  // timeLock config with `new Date().setHours(...)` — the DEVICE's clock,
  // while every other time decision here goes through nowIST(). On a machine
  // not set to IST that expiry landed hours out in either direction, and the
  // screen that would have explained the logout was itself unreachable.
  //
  // It could not actually fire: currentTimeLock was permanently null, so the
  // branch was dead. It is deleted rather than corrected because the feature it
  // belonged to is gone — a wrong-clock computation left standing inside a
  // half-removed feature is what gets switched back on later by someone who
  // assumes it was working.
  //
  // The 8-hour expiry itself is NOT part of that feature and stays: it is the
  // ordinary session timeout, it is what every session has actually been using,
  // and it is measured as a duration from now, so no timezone enters into it.
  // ══════════════════════════════════════════════════════════════════════════
  sessionStorage.setItem(_SESSION_KEY, String(Date.now() + 8 * 60 * 60 * 1000));
}
...
function navigate(view, params={}, pushToStack=true) {
  ...
  if (pushToStack && currentView && currentView !== view) {
    // The params the view being LEFT was rendered with — see currentParams (core.js).
    navStack.push({ view:currentView, params:currentParams || {} });
  }
  currentView   = view;
...
function goBack() {
  ...
  updateBackBtn();
  renderView(prev.view, currentParams);
}
...
function pushNav(view, params={}) {
  ...
  _detachProfileListeners();
  // The params the view being LEFT was rendered with — see currentParams (core.js).
  // This is the push that broke the Fee Card: profile → card → back landed on
  // renderStudentProfile({}) and reported the student did not exist.
  navStack.push({ view:currentView, params:currentParams || {} });
  currentView   = view;
  currentParams = params || {};
  // ══════════════════════════════════════════════════════════════════════════
  // THE SIDEBAR HAS TO SAY WHERE YOU ACTUALLY ARE.
  //
  // navigate() and goBack() both call setActiveNav. pushNav did not, so it moved
  // you to a new view and left the previous section lit — open a student from
  // Terminated and the profile fills the screen while the sidebar still reads
  // TERMINATED. Reported from the live app, and it applies to every pushNav
  // destination, not just that one: Record Payment, Fee Card, Past Due, the
  // add/edit forms.
  //
  // Sub-views like studentProfile have no nav-<id> element, so setActiveNav
  // clears every highlight and lights nothing. That is the honest answer — you
  // are not in a section — and it is what navigate() already does for the same
  // views when they are reached any other way.
  //
  // closeSidebar joins it for the same reason: navigate() closes the drawer after
  // a jump and pushNav left it open over the page it had just rendered.
  // ══════════════════════════════════════════════════════════════════════════
  setActiveNav(view);
  updateBackBtn();
  closeSidebar();
...
function renderView(view, params) {
  switch(view) {
    ...
    // params passed whole as a third argument — the profile needs to know whether
    // it was opened from the Terminated screen, which is the one place a
    // terminated student may be paid. See _flPaymentGuard.
    case 'studentProfile':return renderStudentProfile(params.id, params.studentData, params);
...
function _detachDashListeners() {
  ...
  // AUDIT F5: the shared collection listeners live in that array too, so they
  // have just been unsubscribed. Drop the fan-out registry with them, or the
  // next dashboard mount would register subscribers against a dead listener and
  // every card would sit on its first value forever.
  window._dashFan = null;
}
