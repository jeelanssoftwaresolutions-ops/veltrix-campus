...
function _markConcessionMonths() {
  ...
  document.querySelectorAll('#monthPickerGrid .month-pill').forEach(btn => {
    ...
    btn.innerHTML = `${baseText}<br><span style="font-size:7px;letter-spacing:0.3px;font-weight:700;color:var(--gold-lt)">CONCESSION</span>`;

    // Update tooltip
    btn.title = `${month} — Concession rate ₹${concRate}/month (tap to record payment)`;
  });
}
