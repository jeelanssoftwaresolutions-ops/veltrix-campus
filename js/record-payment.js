  ...
  try {
    ...
    (() => {
      ...
      const _applied = _flAppliedByMonthFromTxs(
        ...
        m => _concessionRateForMonth(m, _stdLD));
  ...
  if (months === 0) {
    ...
    if (info2) {
      ...
      const totalOutstanding = Number.isFinite(_engOut)
        ? _engOut
        : _flPayableForMonths(unpaidPillMonths, m => _concessionRateForMonth(m, info2.rate), _appliedMap0);
...
async function saveFeePayment(prefillId) {
  ...
  if (document.getElementById('feePartialToggle')?.checked) {
    ...
    const _fullMap = _flQuoteByMonth(
      monthsSelected,
      m => _concessionRateForMonth(m, _infoSave.rate),
      window._selectedFeeStudent && window._selectedFeeStudent._appliedByMonth);
