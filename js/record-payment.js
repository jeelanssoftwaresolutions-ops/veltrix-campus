...
function renderRecordFee(params={}) {
  ...
  // Only the Terminated screen's Pay Dues passes fromArchive, and it is re-derived
  // from nav params on EVERY render — arriving here any other way (sidebar,
  // dashboard shortcut, back button) clears it, so the entitlement cannot outlive
  // the one navigation that carried it.
  window._feeArchiveOverride = !!params.fromArchive;
  setContent(`
...
function searchStudentForFee(q) {
  ...
  _feeSearchDebounce = setTimeout(async () => {
    ...
    el.innerHTML = results.map(s=>{
      return `<div class="s-item" onclick="selectFeeStudent('${s.id}','${jsAttr(s.name)}','${s.class} ${s.section}')">
...
async function selectFeeStudent(id, name, cs) {
  // prevBalanceLoading gates the balance field: while it is true, calcLockedFee()
  // shows "Calculating…" instead of painting a figure derived from half-loaded state.
  // It is cleared in the finally below, once the concession rate is known.
  window._selectedFeeStudent = { id, name, cs, prevBalanceLoading: true };
  // BUG-002 FIX: Mark explicit selection.
  // ══════════════════════════════════════════════════════════════════════════
  // F7 — THE FORM NO LONGER UNLOCKS BEFORE THE GUARD HAS ANSWERED.
  //
  // _setFeeWorkflowLock(false) used to run here, immediately on selection, while
  // the student document had not been read yet. The payment guard sits after
  // that read and re-locks on a refusal — so for the length of one Firestore
  // round trip a terminated student's form was open and fillable, which is
  // precisely the window the guard was added to remove. Money was never at risk
  // (saveFeePayment re-checks inside its transaction), but the cost the guard
  // exists to save is the clerk's time, and this was still spending it.
  //
  // The unlock now happens once the guard has passed. See the guard block below.
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // ARC-016's prevBalance FETCH IS GONE. THE GUARD IT CLEARED IS NOT.
  //
  // This used to run a where('studentId').orderBy('date','desc') query, take the
  // newest current-year transaction's frozen remainingBalance, and store it as
  // prevBalance. Nothing ever read it: the two places that would have are the
  // comments in calcLockedFee saying "Do NOT Math.max against prevBalance — that
  // stale Firestore floor prevented concession rates from lowering the balance".
  // The value was removed from the calculation; the query that produced it was not.
  //
  // Reading a transaction's frozen remainingBalance is the same frozen-snapshot
  // pattern the engine exists to replace, so it is not coming back — the balance is
  // computed by calcLockedFee() from the month grid and the operative rate.
  //
  // prevBalanceLoading, however, is load-bearing: calcLockedFee() paints
  // "Calculating…" instead of a figure while it is true. It used to be cleared HERE,
  // which left the entire concession fetch below unguarded — and that fetch is the
  // one the comment beneath calls CRITICAL ORDER, because a calcLockedFee() firing
  // before the concession lands prices the balance at the standard rate. The guard
  // now clears in the finally after the concession is known, which is the window it
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════════════════════
    // TELL THE CLERK NOW, NOT AFTER THEY HAVE FILLED THE FORM IN.
    //
    // Every entry-point button is guarded, but this form is also reachable
    // straight from the sidebar with no student attached — you search, you pick,
    // and nothing so far has looked at who you picked. saveFeePayment WILL refuse
    // (it re-reads the document itself), so the money was never at risk; the cost
    // was selecting the months, entering the amount and only then being told.
    // ══════════════════════════════════════════════════════════════════════
    if (typeof _flPaymentGuard === 'function') {
      const _selGuard = _flPaymentGuard(sDoc2.exists ? sDoc2.data() : null,
                                        { fromArchive: !!window._feeArchiveOverride });
      if (!_selGuard.allowed) {
        showFormAlert('feeAlert', '⛔ ' + _selGuard.reason, 'danger');
        _setFeeWorkflowLock(true);
        window._feeStudentExplicitlySelected = false;
        const _sb = document.getElementById('saveReceiptBtn');
        if (_sb) { _sb.disabled = true; _sb.style.opacity = '0.5'; _sb.style.cursor = 'not-allowed'; }
        return;
      }
    }
    // F7: the guard has answered and this student is payable — open the form now,
    // not a round trip ago. The concession fetch below may still be in flight, but
    // prevBalanceLoading keeps the balance field on "Calculating…" until it lands,
    // so nothing on screen is derived from half-loaded state.
    ...
    if (admNo) {
      ...
      if (!cSnap.empty) {
        ...
        if (typeof overrideFee === 'number' && overrideFee >= 0) {
          ...
          // ══════════════════════════════════════════════════════════════
          // THE CHIP SAID ONE RATE WHILE THE BILL USED ANOTHER.
          //
          // It rendered concessionFee alone — the BASE. But
          // _flConcessionRateForMonth resolves a month's rate as
          // monthlyBreakdown[key] FIRST and falls back to concessionFee only
          // when that month has no per-month entry. Editing a single month
          // through Concession Management writes monthlyBreakdown and leaves
          // concessionFee untouched, so the maths moved to the corrected
          // amount and this chip kept announcing the old one. Reported as
          // "corrected to 1,500, maths uses 1,500, banner still says 1,650",
          // and visible on KABIR KAPOOR as "Concession: ₹1,650/month".
          //
          // A single number cannot describe a per-month schedule, so when one
          // exists the chip stops pretending it can: it shows the range the
          // student is actually billed and says the rate varies. The base is
          // still named when it genuinely is the rate for every covered month.
          // ══════════════════════════════════════════════════════════════
          const infoEl = document.getElementById('selectedStudentInfo');
          if (infoEl) {
            // ══════════════════════════════════════════════════════════════
            // NAME EVERY MONTH AND ITS OWN RATE.
            //
            // A range ("₹1,500–₹1,650 · varies by month") was accurate but not
            // useful: it says the rates differ without saying which month costs
            // what, so a clerk still cannot tell whether the amount they just
            // corrected is the one being applied. "Show the latest applied
            // amount" has no single answer while different months carry
            // different rates — so this stops choosing one and lists them all,
            // each resolved through _flConcessionRateForMonth, the same
            // function that bills them.
            //
            // Ordered by the academic year, June first, so the sequence reads
            // the way the year runs rather than the way the map was written.
            // Falls back to a range only when there are too many to fit.
            // ══════════════════════════════════════════════════════════════
            const _curYrChip = _normaliseAcademicYear(_getCurrentAcademicYearStr());
            const _covered = ACAD_MONTHS_SHORT
              .map(sh => ({ sh, r: _flConcessionRateForMonth(cData, _curYrChip, sh, null) }))
              .filter(x => Number.isFinite(x.r));
            let _label;
            if (!_covered.length) {
              _label = `₹${fmtNum(overrideFee)}/month`;
            } else if (_covered.every(x => x.r === _covered[0].r)) {
              _label = `₹${fmtNum(_covered[0].r)}/month`
                     + (_covered.length < 12 ? ` · ${_covered.length} month${_covered.length === 1 ? '' : 's'}` : '');
            } else if (_covered.length <= 4) {
              _label = _covered.map(x => `${x.sh} ₹${fmtNum(x.r)}`).join(' · ');
            } else {
              const _rs = _covered.map(x => x.r);
              _label = `₹${fmtNum(Math.min.apply(null, _rs))}–₹${fmtNum(Math.max.apply(null, _rs))}/month · ${_covered.length} months`;
            }
            infoEl.innerHTML += ` <span title="Per-month amounts are set in Concession Management and are what the bill uses."
    ...
    // F7: the unlock lives after the guard now, so reaching here means the form is
    // still locked — the student document could not be read, so nothing has
    // confirmed this student is payable. Failing closed is right (saveFeePayment
    // would refuse anyway, after the clerk had filled the form), but a silently
    // dead form is not: say so, and say what to do about it.
    showFormAlert('feeAlert',
      'Could not load this student\'s details, so the form stayed locked. ' +
      'Nothing was recorded. Select the student again to retry.', 'warning');
  } finally {
    // Every exit clears it, including the payment-guard `return` above — a flag left
    // true would strand the balance field on "Calculating…" for the rest of the session.
    if (window._selectedFeeStudent) window._selectedFeeStudent.prevBalanceLoading = false;
  }

  // ══════════════════════════════════════════════════════════════════════════
  // THE CONCESSION LABEL IS NOW APPLIED LAST, BECAUSE IT WAS PAINTED OVER.
  //
  // _markConcessionMonths() ran here, before the pass that fetches paid months
  // and marks the grid. Three markers run after it, and _markDueMonthsRed ends
  // with `btn.textContent = month.slice(0, 3)` — textContent replaces the node's
  // children, so the CONCESSION badge written moments earlier was not restyled,
  // it was deleted.
  //
  // It hit exactly the months where the label matters. A paid month keeps its
  // badge because nothing marks it due; an UNPAID concession month — the one a
  // clerk is about to collect at a reduced rate — lost it every time. Tapping
  // the pill brought it back, which is why this was reported as "concession
  // months don't render highlighted on load, only after clicking in".
  //
  // toggleMonthPill already knew: it re-applies the badge right after resetting
  // textContent, with a comment saying so. The same fix simply never reached the
  // load path.
  //
  // Moved below, after every marker. Its own `dataset.paid === 'true'` guard
  // becomes meaningful there too — at this point nothing had been marked paid
  // yet, so that guard was protecting against a state that could not exist.
  //
  // calcLockedFee stays: it reads concessionRate from the student object, never
  // from the DOM, so it never depended on the badges.
  // ══════════════════════════════════════════════════════════════════════════
  calcLockedFee();

    // The dues banner is rendered by _rpRenderDuesBanner() (module scope, below), called
    // from the END of the month-marking block — NOT on a timer. See that function.
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════════════════════
    // E3 — ONE CURRENT-YEAR OUTSTANDING, COMPUTED ONCE, READ BY EVERY SURFACE.
    //
    // This screen carried two answers to "what does this student still owe this
    // year". The banner took the stored aggregate (outstandingBalance minus
    // previousDues); the Remaining Balance field re-derived it from the DOM
    // pills. Both are plausible and neither is the engine, so they agreed only
    // while the aggregate and the grid agreed — and diverged exactly where those
    // two disagree. Reported as a banner reading 5,700 beside a Remaining
    // Balance of 5,800 for one student at one moment, and as a four-way split
    // once the Profile Card and Record Payment were counted too.
    //
    // The engine is the authority everywhere else, so it is the authority here.
    // Both inputs are already in hand at this point — sData from the guard read,
    // and this year's transactions from the pill pass above — so this costs no
    // extra Firestore read.
    //
    // Stored rather than recomputed per keystroke: the total outstanding does not
    // change as a clerk picks months. What changes is the amount being PAID, and
    // calcLockedFee still derives that from the selection.
    // ══════════════════════════════════════════════════════════════════════
    try {
      const _eYrTx = allTxDocs.filter(t => _flTxBelongsToYear(t, _curYrNorm));
      const _eInfo = _flStudentYearOutstanding(sData, _eYrTx, _curYrNorm, { quiet: true });
      window._selectedFeeStudent.engineOutstanding = Number(_eInfo.outstanding) || 0;
      window._selectedFeeStudent.engineInfo        = _eInfo;
    } catch (_eErr) {
      // Left undefined on purpose. Every reader falls back to its own derivation
      // rather than to zero — a screen that silently says "owes nothing" because a
      // computation failed is how a family stops being billed.
      window._selectedFeeStudent.engineOutstanding = undefined;
      console.warn('[E3] engine outstanding unavailable for ' + id +
        '; banner and balance fall back to their own derivations:', (_eErr && _eErr.message) || _eErr);
    }
    ...
    // JSS-REF-VELTRIX-2026-005 ITEM 14: built by the SHARED _flAppliedByMonthFromTxs, which
    // also credits legacy transactions (no monthAllocations) at the FULL rate. The old inline
    // builder counted allocations ONLY, so a partial month topped up by a normal full payment
    // still read applied < rate forever — reopening a settled month as red/PARTIAL here while
    // _syncStudentFinancials had already (correctly) marked it N/A-PAID.
    (() => {
      const _infoLD = sData.class ? getClassRate(sData.class) : null;
      const _stdLD  = _infoLD ? _infoLD.rate : (sData.monthlyFee || 0);
      const _applied = _flAppliedByMonthFromTxs(
        // AUDIT: through the shared _flTxBelongsToYear. This dropped a transaction
        // carrying no academicYear, while the Profile Card counted the same one as
        // current-year — so an untagged payment showed as collected on the profile
        // and was still quoted for here.
        allTxDocs.filter(t => _flTxBelongsToYear(t, _curYrNorm)),
        // The cap a legacy transaction credits a month at. The engine caps at the
        // month's own price too, so pricing this at the current class rate would
        // over-credit a pre-promotion month by the rate difference.
        _rpRateForMonth(sData, _stdLD));
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // THE LEDGER ALONE CANNOT SAY WHETHER A MONTH IS SETTLED. THE ENGINE CAN.
    //
    // _flPartialMonthsFromLedger asks one question — is 0 < applied < rate — and
    // deliberately does not read the grid. That was right for the problem it was
    // written for (a grid gated to the wrong academicYear never records PARTIAL),
    // but it makes the ledger the WHOLE truth, and it is not.
    //
    // A month closed in full at a lower price has less in the ledger than it now
    // costs, and looks identical to a month that was short-paid. KARAN KAPOOR's
    // July and August hold 1,500 against a 1,700 month — settled in full under the
    // concession in force then, N/A-PAID in the grid, and the engine agrees they
    // owe nothing. This de-lock struck them out of the locked set anyway, so both
    // repainted as PARTIAL 200 and offered themselves for collection.
    //
    // That is the 400 that was reported as a phantom debt, and it was never in the
    // ledger — outstanding was 0 the whole time. It was this screen re-deriving
    // settlement a third way and disagreeing with its own banner.
    //
    // The engine already answers this and E3 publishes the answer. Ask it: a month
    // it calls paid or excused is closed, whatever the ledger total looks like next
    // to today's rate. The ledger derivation still governs everything else, so the
    // Bulk-Admit case it was written for keeps working.
    // ══════════════════════════════════════════════════════════════════════════
    (() => {
      ...
      const _engDL  = window._selectedFeeStudent && window._selectedFeeStudent.engineInfo;
      const _settledDL = m => !!(_engDL
        && ((_engDL.paid    && (_engDL.paid.has(m)    || _engDL.paid.has(_flShort(m))))
         || (_engDL.excused && (_engDL.excused.has(m) || _engDL.excused.has(_flShort(m))))));
      // Same price the pill label and the quote use, or a month could be un-locked
      // here as partial and then quoted as though it were whole.
      _flPartialMonthsFromLedger(_appliedDL, _rpRateForMonth(sData, _stdDL))
        .forEach(m => { if (!_settledDL(m)) paidMonths.delete(m); });
    })();
    ...
    (() => {
      ...
      // Through the engine's price, not the current class rate — a pre-promotion
      // month costs what it cost then, and this label is what the clerk collects on.
      const _rateOf = _rpRateForMonth(sData, _std);
      document.querySelectorAll('#monthPickerGrid .month-pill').forEach(p => {
        ...
        const left = Math.max(0, _rateOf(m) - a);
        if (left <= 0) return;
    ...
    // Every marker has run, so the concession badges can be applied without one of
    // them deleting the label afterwards. LAST, deliberately — see the block above
    // where this call used to sit.
    _markConcessionMonths();

    // Every pill's paid / excused / partial state is settled by this point. Render the
    // banner HERE, as the last step of the work it depends on, instead of guessing at 150 ms.
    _rpRenderDuesBanner(sData);
  } catch(e) {
    console.warn('[ARC-016] Paid months fetch failed:', e.message);
    // The markers may not have run, but a concession is still a concession — label
    // what is there rather than leaving a student's reduced rate invisible.
    try { _markConcessionMonths(); } catch(_) {}
    // Still render, so a fetch failure shows the stored aggregate rather than a stale banner.
    try { _rpRenderDuesBanner(sData); } catch(_) {}
  }
}

// ════════════════════════════════════════════════════════════════════════════
// JSS-REF-VELTRIX-2026-005 — "Total Outstanding" banner. Rewritten; it had THREE
// independent defects that compounded into one wrong number, on the same screen
// that was simultaneously displaying the correct month grid.
//
// Reported live for ADM-TEST-002: banner ₹20,400 / "12 unpaid months" while the grid
// directly beneath it showed 6 PAID and 6 DUE. 20,400 = 12 × 1,700.
//
//  1. IT RACED THE DATA IT DEPENDED ON. The old code ran inside setTimeout(..., 150)
//     and scraped `dataset.paid` off the pills. Those flags are set by
//     _markPaidMonths() at the end of an AWAITED feeTransactions query. Whenever that
//     query took longer than 150 ms — routine on real data — every pill still read
//     paid='false', so the banner counted all twelve months. It then never recomputed,
//     so the grid corrected itself underneath a banner frozen at its wrong first guess.
//     The 150 ms WAS the bug: a timer standing in for a dependency.
//
//  2. IT PRICED FROM A STALE CLASS. getClassRate(window._selectedFeeStudent.cs) reads
//     the classSection cached on the in-memory selection object rather than the student
//     document. For the reported student that resolved to a ₹1,700 class (Nursery–Grade 5)
//     while the document says Grade 8 at ₹1,800.
//
//  3. IT WAS A FOURTH DEFINITION OF OUTSTANDING. Summing pills is not what any other
//     screen does, so it could only ever agree with them by coincidence — exactly what
//     items 4/16/17 forbid.
//
// Now: the authoritative current-year figure is outstandingBalance − previousDues, the
// same expression the Rolling Dues card uses (item 9), read from the student DOCUMENT.
// The pill sum survives only as a fallback for a student whose aggregate has never been
// written, and even then it runs after marking completes and prices from the document.
// ════════════════════════════════════════════════════════════════════════════
function _rpRenderDuesBanner(sData) {
  const el = document.getElementById('_ctxDuesBanner');
  if (!el) return;
  try {
    ...
    const _agg     = Number(sData && sData.outstandingBalance);
    const _prev    = Number(sData && sData.previousDues) || 0;
    const _haveAgg = Number.isFinite(_agg);

    // Fallback only — price from the DOCUMENT's class, never the cached selection.
    const _docCls  = (sData && (sData.class || sData.cls)) || '';
    const _stdRate = (_docCls ? _flRateForClass(_docCls, 0) : 0)   // L2/L8: one resolver
                     || (sData && sData.monthlyFee) || 0;
    const _pillDue = _unpaidPills.reduce(
      (sum, b) => sum + _concessionRateForMonth(b.dataset.month, _stdRate), 0);

    // E3: the engine's answer first — the same number the Remaining Balance field
    // and the Profile Card now read. The aggregate subtraction below is kept only
    // as a fallback for the case where the engine could not run at all; it is what
    // used to make this banner disagree with the field beside it.
    const _eng = window._selectedFeeStudent && window._selectedFeeStudent.engineOutstanding;
    const _totalDue = Number.isFinite(_eng) ? _eng
                    : (_haveAgg ? Math.max(0, _agg - _prev) : _pillDue);

    el.style.display = 'block';
    if (_totalDue > 0) {
      const _prevNote = _prev > 0
        ? ` <span style="color:var(--muted)">· plus ₹${fmtNum(_prev)} from previous years</span>` : '';
      el.innerHTML = `<span style="color:var(--danger);font-size:12px">⚠️ Total Outstanding: ` +
        `<strong>₹${fmtNum(_totalDue)}</strong> — <span style="color:var(--muted)">` +
        `${_unpaidPills.length} unpaid month${_unpaidPills.length !== 1 ? 's' : ''} in ` +
        `${_getCurrentAcademicYearStr()}</span>${_prevNote}</span>`;
    } else {
      el.innerHTML = `<span style="color:var(--success);font-size:12px">✅ No outstanding dues for the current academic year (${_getCurrentAcademicYearStr()})</span>`;
    }
    calcLockedFee();
    if (window._selectedFeeStudent) window._selectedFeeStudent.liveDue = _totalDue;
  } catch (e) {
    console.warn('[DUES BANNER] render failed:', e && e.message);
  }
}
...
function _markPriorGradeDueMonths(dueMonths, fromClass, paidMonths) {
  document.querySelectorAll('#monthPickerGrid .month-pill').forEach(btn => {
    ...
    // ITEM 5 OPTION B: provenance label only — this month predates the promotion,
    // but it is billed at the CURRENT class rate like every other month. The old
    // copy promised the prior grade's rate, which is no longer what is charged.
    btn.title = `${month} — carried over unpaid from ${fromClass}, billed at the current class rate`;
    btn.innerHTML = `${month.slice(0,3)}<br><span style="font-size:7px;letter-spacing:0.3px;font-weight:700;color:var(--danger)">CARRIED OVER</span>`;
  });
}
...
function _fixStudentFeeRates(s) {
  ...
  if (Array.isArray(s.openingOutstandingDues) && s.openingOutstandingDues.length > 0) {
    ...
    s.openingOutstandingDues = s.openingOutstandingDues.map(due => {
      // ══════════════════════════════════════════════════════════════════════
      // L1 — DO NOT GUESS WHAT IS ALREADY RECORDED.
      //
      // Everything below this guard is inference: reverse-engineer a month count
      // by dividing a rupee total by TODAY's rate, guess which class the student
      // was in N years ago by walking the promotion chain backwards, multiply the
      // two, and overwrite the stored amount with the product.
      //
      // Every step of that is now unnecessary where the data exists, and wrong
      // where it does not:
      //   · Math.round(amount / currentRate) cannot survive a partial month, a
      //     concession month, or an amount that was never a whole multiple of any
      //     rate. It silently rounds to a plausible-looking integer.
      //   · _classNYearsBefore assumes one promotion per year, every year, no
      //     repeats. On this roll 145 students were promoted once while their
      //     academicYear never moved, so the year gap and the class history
      //     disagree by construction.
      //
      // Since 54d9d88 an entry carries its own monthlyFee, and the engine prices
      // its monthStatus grid month by month at that rate. Where either exists,
      // the record IS the answer and this heuristic must keep its hands off.
      // ══════════════════════════════════════════════════════════════════════
      const _hasGrid       = !!(due && due.monthStatus && Object.keys(due.monthStatus).length);
      const _hasStoredRate = Number(due && due.monthlyFee) > 0;
      if (_hasGrid || _hasStoredRate) { correctedPrevTotal += (due.amount || 0); return due; }
      ...
      // Only reached when the entry carries NEITHER a grid nor a rate — a genuinely
      // unrecorded year, where an inferred figure beats no figure.
  ...
  // L1 — same guard. previousDues is an AGGREGATE that _syncStudentFinancials
  // recomputes from the month grids and the allocation ledger. Where a prior-year
  // grid exists, or the year is described by an openingOutstandingDues entry, that
  // aggregate is derived from real per-month records and this reverse-engineered
  // figure would overwrite fact with inference. Only a year with no grid and no
  // entry anywhere is a genuine blank worth guessing at.
  const _prevGridExists = !!(
    (s.previousYearMonthStatus && Object.keys(s.previousYearMonthStatus).length) ||
    (s.prevYearMonthStatus     && Object.keys(s.prevYearMonthStatus).length)     ||
    (Array.isArray(s.openingOutstandingDues) && s.openingOutstandingDues.some(
      d => d && ((d.monthStatus && Object.keys(d.monthStatus).length) || Number(d.monthlyFee) > 0)))
  );
  if ((s.previousDues || 0) > 0 && currentRate && !_prevGridExists) {
...
function _resetMonthPills() {
  ...
  const ACAD_MONTHS = ACAD_MONTHS_FULL;
...
function _markConcessionMonths() {
  ...
  // Same ONE key builder _concessionRateForMonth uses. This used to hand-roll the
  // Jun–Dec/Jan–May span rule itself, so the LABEL on a pill and the RATE charged for
  // it were derived independently and could disagree.
  ...
  document.querySelectorAll('#monthPickerGrid .month-pill').forEach(btn => {
    ...
    const key   = (typeof _flConcMonthKey === 'function') ? _flConcMonthKey(_acadYrStr, month) : '';
    if (!key) return;
...
function _markDueMonthsRed(dueMonths, paidMonths) {
  document.querySelectorAll('#monthPickerGrid .month-pill').forEach(btn => {
    ...
    // ══════════════════════════════════════════════════════════════════════
    // A PART-PAID MONTH IS NOT PLAINLY "DUE", AND MUST NOT BE REPAINTED AS ONE.
    //
    // This function ends with `btn.textContent = month.slice(0, 3)`, which
    // replaces the node's children. The pill pass above stamps a partial month
    // with "PARTIAL ₹1,200" and amber styling; this ran afterwards and deleted
    // both, leaving a plain red month indistinguishable from one nothing has
    // been paid against.
    //
    // That is a money-visible error, not a cosmetic one: the clerk sees a normal
    // unpaid month and collects the FULL rate instead of the remainder, and the
    // family is charged twice for the part already settled. Same deletion that
    // took the concession badge — a month can carry both, and both were lost.
    //
    // dataset.partial is stamped from the live allocation ledger, so trusting it
    // here reads the same source the amount does.
    // ══════════════════════════════════════════════════════════════════════
    if (btn.dataset.partial === 'true') return;
...
function toggleMonthPill(btn) {
  ...
  const _ACAD_MONTHS_TMP = ACAD_MONTHS_FULL;
...
// ════════════════════════════════════════════════════════════════════════════
// WHAT ONE MONTH COSTS ON THIS SCREEN — AND IT IS THE ENGINE'S ANSWER.
//
// Option B (below) decided every month of the year is billed at the CURRENT class
// rate, and recorded as fact that "the fee engine ... has always billed every month
// of the year at the year's current rate". That was true when it was written. It
// stopped being true when the mid-year promotion fix landed: the engine's CHARGE
// moved onto _flHistoricalRateForMonth, so a month before the promotion's effective
// month is now billed at the rate that was in force then.
//
// Nothing brought this file along, so the two have been holding opposite policies.
// KARAN KAPOOR after promoting to Grade 6 (1,700 -> 1,800), with July carrying
// 1,500 of a Grade 1 month: the pill said PARTIAL 300 and the engine said 200. The
// screen's own banner, which reads the engine, disagreed with the tiles beneath it.
//
// The engine wins — it is the authority everywhere else, and the app's own
// promotion banner promises "Grade 6 fee cycle begins October", which billing July
// at 1,800 contradicts. engineInfo.rateForMonth is that exact function, already
// published on this screen by E3 and until now unread. The composition below is the
// fallback for when the engine could not run, and it is deliberately the same two
// resolvers in the same order, so the fallback cannot drift from what it stands in for.
// ════════════════════════════════════════════════════════════════════════════
function _rpRateForMonth(sData, standardRate) {
  const _eng = window._selectedFeeStudent && window._selectedFeeStudent.engineInfo;
  if (_eng && typeof _eng.rateForMonth === 'function') {
    return m => {
      const r = Number(_eng.rateForMonth(m));
      return Number.isFinite(r) && r >= 0 ? r : _concessionRateForMonth(m, standardRate);
    };
  }
  let _at = null;
  try {
    if (typeof _flHistoricalRateForMonth === 'function') {
      _at = _flHistoricalRateForMonth(sData, _getCurrentAcademicYearStr(), standardRate);
    }
  } catch (_) { _at = null; }
  return m => _concessionRateForMonth(m, _at ? (Number(_at(m)) || standardRate) : standardRate);
}

function _concessionRateForMonth(monthName, standardRate, yr) {
  const student = window._selectedFeeStudent;

  // ══════════════════════════════════════════════════════════════════════════
  // JSS-REF-VELTRIX-2026-005 ITEM 5, OPTION B — the prior-grade rate override is
  // GONE. Every month of the year is billed at the CURRENT class rate.
  //
  // JSS-REF-013 used to return student.priorGradeRate here, so a month left
  // unpaid before a mid-year promotion was quoted at the OLD class's rate. That
  // put this quote in direct conflict with the fee engine, which has always
  // billed every month of the year at the year's current rate — so Record Payment
  // quoted ₹1,700, the student paid ₹1,700, and the engine went on believing the
  // month owed ₹1,800. It settled as PARTIAL ₹100 and could never be closed.
  //
  // Item 5 requires already-PAID months to top up to the new class fee. Leaving
  // unpaid months at the old rate would have meant the same calendar month is
  // priced differently depending on whether it happened to be paid before the
  // promotion — the asymmetry flagged when item 5 shipped. Option B removes it:
  // ONE billing rate for the whole year, the new class's.
  //
  // What survives from JSS-REF-013 is provenance only: priorGradeDueMonths still
  // labels those pills so staff can see which months predate the promotion. It no
  // longer changes what they cost. Concession now applies to them like any other
  // month, since they are ordinary current-class months as far as billing goes.
  //
  // The CREDIT side is unaffected and still uses the historical rate — see
  // _flHistoricalRateForMonth. Billing is uniform; what was already paid is
  // credited at what was actually paid. That is what produces the difference.
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ONE month-key builder. The Jun–Dec/Jan–May span rule was hand-written here, in
  // _markConcessionMonths below, and a third time as _flConcMonthKey in the engine —
  // three copies of one rule, which is how the engine and this surface end up
  // disagreeing about which months a concession covers.
  //
  // `yr` is optional and defaults to the current academic year, which is correct for
  // every caller today: this function is only reached from Record Payment, and every
  // call site there is scoped to the current AY. Past Due Recording does not use it.
  // The parameter exists so a prior-year caller CANNOT silently get the wrong year —
  // it has to pass one.
  if (typeof _flConcMonthKey !== 'function') {
    // Falling back to the standard rate here would quietly CHARGE A CONCESSION
    // STUDENT FULL PRICE. Say so — a load-order failure must not read as "this
    // student has no discount".
    if (!window._rpConcKeyWarned) {
      window._rpConcKeyWarned = true;
      console.error('[CONCESSION] _flConcMonthKey is unavailable, so concession months ' +
        'cannot be resolved and every month is quoting the STANDARD rate. pending-fee.js ' +
        'has not loaded. Do not take payments until this is fixed.');
    }
    return standardRate;
  }
  const key = _flConcMonthKey(yr || _getCurrentAcademicYearStr(), monthName);
  if (!key) return standardRate; // unknown month name → safe fallback
...
function calcLockedFee() {
  ...
  if (months === 0) {
    ...
    if (info2) {
      ...
      // E3: one outstanding for this screen. The pill derivation stays as the
      // fallback — it is correct whenever the grid and the engine agree, and it is
      // all there is if the engine could not run — but the engine's figure wins so
      // this field and the banner above it can no longer show different numbers.
      const _engOut = window._selectedFeeStudent && window._selectedFeeStudent.engineOutstanding;
      const totalOutstanding = Number.isFinite(_engOut)
        ? _engOut
        : _flPayableForMonths(unpaidPillMonths, _rpRateForMonth(null, info2.rate), _appliedMap0);
  ...
  // The month's price per the engine — see _rpRateForMonth. Quoting info.rate here
  // billed a pre-promotion month at the NEW class rate while the banner above it
  // (engineOutstanding) priced the same month at the old one.
  const _rateOfMonth = _rpRateForMonth(null, info.rate);
  const _quoteForMonth = m => {
    const r = _rateOfMonth(m);
  ...
  // E3: the fourth reader of this one number, and the last. This branch runs once
  // months ARE selected, and fed "outstanding X − paying Y = Z remaining" from its
  // own pill sum while the branch above and the banner used other derivations —
  // which is how one student showed four different balances at one moment.
  // _quoteForMonth stays the fallback; it prices correctly whenever the grid and
  // the engine agree, and it is all there is if the engine could not run.
  const _engOutSel = window._selectedFeeStudent && window._selectedFeeStudent.engineOutstanding;
  const totalOutstanding = Number.isFinite(_engOutSel)
    ? _engOutSel
    : allUnpaidPillMonths.reduce((sum, m) => sum + _quoteForMonth(m), 0);
  // LIVE-WIRE: expose to profile card
  if (window._selectedFeeStudent) window._selectedFeeStudent.liveDue = totalOutstanding;
...
async function saveFeePayment(prefillId) {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // TERMINATION GUARD — THE WRITE, NOT THE BUTTON.
  //
  // Every entry point now hides Record Payment for a terminated student, and none
  // of that is a guard: a stale page, the back button, a half-loaded render or a
  // console call all reach this function with the button never consulted. So the
  // student document is re-read at save time, inside the write transaction.
  //
  // Not from window._selectedFeeStudent and not from any cache — the whole point
  // is to be immune to what the page thinks it knows. A student terminated in
  // another tab while this form sat open is exactly the case that has to fail.
  //
  // AUDIT F8 — THE CHECK LIVES WHERE THE WRITE LIVES.
  //
  // The first version of this guard read the student document here, several
  // hundred lines above the write, and the write then went out as a batch.
  // A batch is atomic but reads nothing, so between the check and the commit
  // there was a window in which a termination could land and the payment still
  // went through. Small, real, and precisely the kind of gap a guard is supposed
  // not to have.
  //
  // The read now happens inside runTransaction alongside the two writes, so
  // Firestore itself guarantees the document did not change in between: if it
  // did, the transaction retries and the guard is asked again. See the write
  // block near the end of this function.
  //
  // There is no way past this. The archive exception that shipped with the first
  // version of this guard is gone: a terminated student is not payable from any
  // screen, by any role. Undo Terminate, take the payment, terminate again.
  // ══════════════════════════════════════════════════════════════════════════
  ...
  if (student.cs) {
    ...
    if (info) {
      ...
      // The engine's per-month price — this is the amount actually collected, so
      // pricing a pre-promotion month at the new class rate would over-charge at
      // the counter, not merely mis-display.
      lockedAmount = _flPayableForMonths(monthsSelected, _rpRateForMonth(null, info.rate),
  ...
  if (!lockedAmount) {
    try {
      ...
      if (info) {
        ...
        lockedAmount = _flPayableForMonths(monthsSelected, _rpRateForMonth(sd, info.rate),
                       window._selectedFeeStudent && window._selectedFeeStudent._appliedByMonth);
  ...
  const ACAD_MONTHS_SAVE = ACAD_MONTHS_FULL;
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // THE RECEIPT'S REMAINING BALANCE IS THE ENGINE'S ANSWER MINUS THE CASH TAKEN.
  //
  // This used to scrape #monthPickerGrid and price every still-unpaid pill at its
  // FULL month rate. Two things were wrong with that, and both reached the parent:
  //
  //   1. A PARTIAL month owes its REMAINDER, not the whole fee. A July already
  //      carrying 1,500 of a 1,700 month was counted at 1,700, so every receipt
  //      issued afterwards overstated the balance. Karan Kapoor's Nov+Dec receipt
  //      printed 12,000 where the true figure was 8,600 — the 3,400 gap is exactly
  //      July and August counted whole instead of at their 200 remainders.
  //   2. It was a SECOND source of truth. E3 gave this screen one outstanding
  //      (engineOutstanding) and wired the banner and the Remaining Balance field
  //      to it; saveFeePayment was the one surface left re-deriving its own, which
  //      is why the field read 8,500 and the receipt read 12,000 at one moment.
  //
  // _syncStudentFinancials repairs the student document a moment later, so the
  // LEDGER healed itself and only the receipt stayed wrong — and a receipt is
  // handed over, filed, and never recomputed. That is why this had to be fixed at
  // the source instead of left to the reconcile.
  //
  // The identity below is the one the Remaining Balance field already uses:
  //   remaining = outstanding before this payment − cash actually applied now
  // so the field and the receipt can no longer print different numbers. The pill
  // derivation stays as the fallback for when the engine could not run, now
  // netting partial months through the shared _flPayableForMonths helper.
  // ══════════════════════════════════════════════════════════════════════════
  const _outstandingBefore = (() => {
    const _eng = window._selectedFeeStudent && window._selectedFeeStudent.engineOutstanding;
    if (Number.isFinite(_eng)) return _eng;
    if (!_infoSave) return 0;
    const _appliedMapSave = (window._selectedFeeStudent && window._selectedFeeStudent._appliedByMonth) || {};
    const _unpaidSave = Array.from(document.querySelectorAll('#monthPickerGrid .month-pill'))
      .filter(b => b.dataset.paid !== 'true' && !b.disabled)
      .map(b => b.dataset.month);
    return _flPayableForMonths(_unpaidSave, _rpRateForMonth(null, _infoSave.rate), _appliedMapSave);
  })();
  let balance = 0;
  ...
  if (_flIsDeparted(sData) && currentRole !== 'principal') {
  ...
  if (document.getElementById('feePartialToggle')?.checked) {
    const _rateForMonthSave = _rpRateForMonth(null, _infoSave ? _infoSave.rate : 0);
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // JSS-REF-VELTRIX-2026-005 ITEM 14 — a FULL payment now writes monthAllocations too.
    //
    // Previously the map was written ONLY on partial payments, so the ledger recorded
    // the short first instalment on a month and NOTHING for the full top-up that closed
    // it. Any consumer reading allocations alone therefore saw applied < rate forever and
    // reopened an already-settled month as red/PARTIAL — the exact defect reported (pay
    // months → concession → partial on a concession month → clear it → pay on, and the
    // cleared month comes back).
    //
    // The map is _flQuoteByMonth — the SAME per-month computation lockedAmount is the sum
    // of — so the allocations and the recorded amount are one calculation, not two that
    // can drift. Written only when it reconciles to `amount` exactly; on any mismatch we
    // fall back to omitting it (legacy behaviour) rather than persisting a ledger that
    // disagrees with the receipt.
    // ══════════════════════════════════════════════════════════════════════════
    const _fullMap = _flQuoteByMonth(
      monthsSelected,
      _rpRateForMonth(null, _infoSave.rate),
      window._selectedFeeStudent && window._selectedFeeStudent._appliedByMonth);
    const _mapSum = Object.keys(_fullMap).reduce((s, m) => s + _fullMap[m], 0);
    if (Object.keys(_fullMap).length && Math.abs(_mapSum - amount) < 0.5) {
      monthAllocations = _fullMap;
      partialMonths    = [];          // a full payment closes every month it touches
      monthShortage    = {};
    }
  }

  // ONE derivation, after both branches have settled what `amount` actually is.
  // A short payment leaves its shortfall behind automatically: the months being paid
  // are still inside _outstandingBefore, and only the cash collected comes off it.
  balance = Math.max(0, _outstandingBefore - (Number(amount) || 0));

  // ══════════════════════════════════════════════════════════════════════════
  // WHAT EACH MONTH COST ON THE DAY IT WAS PAID. THE LEDGER NEVER RECORDED IT.
  //
  // A transaction stored monthsSelected and amountPaid "and nothing about the rate
  // each month carried" — _txConcessionMonths says so in as many words, and derives
  // the concession label live from today's register because there was nothing else
  // to read. So "was this month settled in full?" could only ever be re-answered
  // against TODAY's price, and any later change to that price rewrote the answer.
  //
  // KARAN KAPOOR: July and August were paid in full at a 1,500 concession. A second
  // concession was later applied to November and December — and a student holds ONE
  // concession document, so that edit overwrote activeMonths wholesale. July and
  // August silently lost their concession, repriced to 1,700, and the 1,500 already
  // collected became a partial owing 200 each. 400 of debt the school never charged,
  // on two months it had already closed and forgiven the difference on.
  //
  // The engine has _flHistoricalRateForMonth so a CLASS rate change can't reopen a
  // settled month. This is the same guarantee for the concession dimension, built
  // the same way the app treats every other financial fact: written down when it
  // happens rather than re-derived afterwards. A receipt records the price.
  //
  // Additive and keyed by month name, so legacy transactions keep their exact
  // current behaviour — a month with no recorded rate makes no closure claim.
  // ══════════════════════════════════════════════════════════════════════════
  let monthRates = null;
  if (_infoSave && Array.isArray(monthsPaidList) && monthsPaidList.length) {
    monthRates = {};
    const _rateNow = _rpRateForMonth(null, _infoSave.rate);
    monthsPaidList.forEach(m => {
      const r = Number(_rateNow(m));
      if (Number.isFinite(r) && r > 0) monthRates[m] = r;
    });
    if (!Object.keys(monthRates).length) monthRates = null;
  }
  ...
  const tx = {
    ...
    // Was an inline copy of the IST wall-clock expression (BUG-TS-001), duplicated in
    // three files. Same instant, now via the one shared converter.
    paymentModeBreakup, date: istTimestampFromDateInput(date, 'now'),
    ...
    // The price each month carried on this date — see the block above.
  ...
  try {
    ...
    // 1) The fee transaction document. The ref is created OUTSIDE the
    //    transaction so a Firestore retry re-uses the same document id and
    //    cannot leave a duplicate receipt behind.
    ...
    // _syncStudentFinancials recomputes this authoritatively immediately after the write.
    const newFeeStatus = balance <= 0 ? 'paid' : (paymentStatusFinal === 'Partial' ? 'partial' : 'pending');

    // ══════════════════════════════════════════════════════════════════════
    // AUDIT F8 — was db.batch(). A batch is atomic but reads nothing, so the
    // termination check had to happen earlier and separately, leaving a window
    // where a termination could land between the check and the commit.
    //
    // runTransaction reads the student INSIDE the same atomic unit as the two
    // writes: if that document changes in between, Firestore aborts and retries,
    // and the guard is asked again against the new state. The callback is
    // idempotent under retry — txRef is fixed above, and the payload is computed
    // before we get here.
    // ══════════════════════════════════════════════════════════════════════
    await db.runTransaction(async t => {
      const _stuSnap = await t.get(stuRef);

      // A student that no longer exists is not payable. The display guard fails
      // open on a missing document, which is right for deciding whether to draw
      // a button and wrong for deciding whether to take money.
      if (!_stuSnap.exists) {
        throw new Error('This student record no longer exists, so the payment was not recorded.');
      }
      if (typeof _flPaymentGuard === 'function') {
        const _guard = _flPaymentGuard(_stuSnap.data(), { fromArchive: !!window._feeArchiveOverride });
        if (!_guard.allowed) throw new Error(_guard.reason);
      }
      ...
      t.update(stuRef, {
        ...
        lastPaymentDate:    istTimestampFromDateInput(date, 'now'),
        lastPaymentAmount:  amount
      });
    });

    // ITEM-10 FIX: canonical cross-module reconciliation as the final step.
    // F2: and its answer is READ. It used to be awaited and discarded, which —
    // because the sync swallows its own error — meant a failed reconcile was
    // indistinguishable from a successful one right here, and the clerk was told
    // "Payment recorded successfully!" over stale totals.
    const _syncRes = await _syncStudentFinancials(student.id);
    const _syncFailed = !!(_syncRes && _syncRes.ok === false);
    ...
    auditLog('fee_payment_recorded', {
      ...
      // F2: on the record, not just on screen. A receipt whose reconcile failed
      // is the one a later investigation needs to find, and the clerk's alert is
      // gone the moment they navigate away.
      ...(_syncFailed ? { syncFailed: true, syncError: (_syncRes && _syncRes.error) || 'unknown' } : {})
    });

    // ══════════════════════════════════════════════════════════════════════
    // THE MONEY IS SAFE. THE TOTALS ARE NOT. SAY BOTH, AND SAY WHICH.
    //
    // The transaction above committed the receipt and the student's balance
    // together, so on this path the payment IS recorded and the receipt IS
    // valid — it must still be shown and handed over. What failed is the
    // reconcile that refreshes the aggregates every other screen reads.
    //
    // The danger here is not the stale figure; previewHealthCheck finds those
    // and the next successful sync clears them. The danger is a clerk who sees
    // Due Fee unchanged and records the payment a SECOND time. So the warning
    // leads with the receipt being valid and says plainly not to re-enter it.
    // ══════════════════════════════════════════════════════════════════════
    if (_syncFailed) {
      showFormAlert('feeAlert',
        `Payment recorded — receipt ${tx.receiptNumber} is valid and can be handed over. ` +
        `But this student's totals could NOT be refreshed (${(_syncRes && _syncRes.error) || 'unknown error'}). ` +
        `Due Fee, the profile card and the dashboard will keep showing the OLD outstanding until that succeeds. ` +
        `Do NOT record this payment again — it is already saved. ` +
        `Re-open the student to retry the refresh, or run await previewHealthCheck() in the console.`,
        'warning');
    } else {
    ...
    // A refusal is not a malfunction, and "Error:" in front of it reads as one.
    // The termination guard and the missing-student check both throw a sentence
    // written for the clerk; show it as-is. Anything else is a genuine fault and
    // keeps the prefix.
    const _msg = (e && e.message) || 'unknown';
    const _refused = /TERMINATED|no longer exists/i.test(_msg);
    showFormAlert('feeAlert', _refused ? '⛔ ' + _msg : 'Error: ' + _msg, 'danger');
...
async function showReceipt(tx) {
  // R1: prime the concession register so _txConcessionMonths can tag which months
  // this payment covered at a discounted rate. All four callers invoke showReceipt
  // as a fire-and-forget statement, so awaiting here is safe.
  if (typeof _flLoadConcessions === 'function') { try { await _flLoadConcessions(); } catch(_){} }
  ...
  // R1: mark each month that was billed at a concession rate, alongside partial.
  const _concMonths = (typeof _txConcessionMonths === 'function') ? _txConcessionMonths(tx) : [];
  const _monthsLabel = (tx.monthsSelected && tx.monthsSelected.length > 0)
    ? tx.monthsSelected.map(m => {
        const _tags = [];
        if (_partMonths.includes(m)) _tags.push('partial');
        if (_concMonths.includes(m)) _tags.push('concession');
        return _tags.length ? `${m} (${_tags.join(', ')})` : m;
      }).join(', ')
  ...
  // The PDF builder used to reconstruct this receipt by SCRAPING the DOM above —
  // reading a financial document back off the screen it had just painted. It looked
  // for an "Amount Paid" label/value pair, which does not exist: the amount lives in
  // #rc_total as one combined string. The lookup missed, parseFloat('—') gave NaN,
  // `|| 0` turned that into zero, and every downloaded receipt printed INR 0.00 for a
  // real payment. Test Student Twelve's 14,400 receipt is the one that surfaced it.
  // Keep the transaction itself; a receipt is built from the record, not the pixels.
  window._lastReceiptTx = tx;
...
function printReceipt() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // BUILD THE RECEIPT FROM THE TRANSACTION, NOT FROM THE SCREEN.
  //
  // Every field here was scraped out of the rendered DOM. For the money field that
  // silently failed: val('amount paid') looks for a .r-lbl/.r-val pair, the amount
  // is written into #rc_total as the single string "Amount Paid: ₹14,400", the
  // lookup returned '—', parseFloat gave NaN and `|| 0` printed INR 0.00. A real
  // 14,400 payment downloaded as a zero-rupee receipt, and nothing anywhere warned.
  //
  // The DOM scrape survives only as a fallback for a receipt shown by some path
  // that never stashed its transaction — and it can no longer turn a missing value
  // into a confident zero.
  // ══════════════════════════════════════════════════════════════════════════
  const _tx = window._lastReceiptTx || null;
  const _num = v => { const n = parseFloat(String(v == null ? '' : v).replace(/[^\d.]/g, '')); return Number.isFinite(n) ? n : null; };
  const _scrapedPaid = _num(val('amount paid'));
  const _paid = _tx && Number.isFinite(Number(_tx.amountPaid)) ? Number(_tx.amountPaid) : _scrapedPaid;

  if (_paid == null) {
    showToast('Cannot build the receipt — the payment amount could not be read. ' +
              'Re-open the receipt from Paid Fee and try again.', 'danger');
    console.error('[RECEIPT] Refusing to print: amountPaid unavailable from both the ' +
      'transaction and the DOM. Printing 0.00 would look like a genuine zero-rupee receipt.');
    return;
  }

  // R1: the receipt PDF has no month grid — it shows one fee-particulars line — so the
  // concession is surfaced there. showReceipt primed the register before this runs.
  const _pdfConc = (typeof _txConcessionMonths === 'function' && _tx) ? _txConcessionMonths(_tx) : [];
  const _pdfFeeHead = ((_tx && _tx.feeHead) || val('fee head') || 'Academic Operational Fees')
    + (_pdfConc.length ? '  (Concession applied: ' + _pdfConc.map(m => String(m).slice(0,3)).join(', ') + ')' : '');

  const txData = {
    receiptNumber  : (_tx && _tx.receiptNumber) || val('receipt'),
    studentName    : (_tx && _tx.studentName)   || val('student'),
    admissionNo    : (_tx && (_tx.admissionNumber || _tx.admissionNo)) || val('adm'),
    classSection   : (_tx && _tx.classSection)  || val('class'),
    date           : val('date'),
    paymentMode    : (_tx && _tx.paymentMode)   || val('mode'),
    paymentStatus  : (_tx && _tx.paymentStatus) || val('status') || 'COMPLETED',
    feeHead        : _pdfFeeHead,
    academicYear   : (_tx && _tx.academicYear)  || '',
    monthsSelected : (_tx && _tx.monthsSelected) || null,
    amountPaid     : _paid,
    remainingBalance: (_tx && Number.isFinite(Number(_tx.remainingBalance)))
                        ? Number(_tx.remainingBalance)
                        : (_num(val('remaining')) ?? _num(val('balance')) ?? 0),
  };
