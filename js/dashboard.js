...
async function renderAdminDash() {
  ...
  const unsubFees = schoolCol('feeTransactions')
    .onSnapshot(snap => {
      ...
      const total = snap.docs.reduce((s,d) => { const dd=d.data(); return dd.isHiddenPayment ? s : s + (dd.amountPaid || 0); }, 0);
...
async function renderPrincipalDash() {
  ...
  const unsubFees = schoolCol('feeTransactions')
    .onSnapshot(snap => {
      const total = snap.docs.reduce((s,d) => s + (d.data().amountPaid || 0), 0);
