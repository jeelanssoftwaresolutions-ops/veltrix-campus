...
async function renderPrincipalDash() {
  ...
  (() => {
    ...
    //
    // This line classified on age alone, so a run whose checks FAILED rendered
    // exactly like a clean one — "run today", in calm grey — and the comment
    // that used to sit here said amber never means "something is wrong,
    // because nothing here has detected a fault". By then something had.
    //
    // Outcome now leads and age follows it. Three states, because there are
    // genuinely three: a recorded failure, a recorded pass, and no verdict at
    // all. The third covers a browser that has never run the checks AND a
    // record written before the verdict was stored — both are UNKNOWN, and
    // unknown must never borrow the styling of healthy.
    // ══════════════════════════════════════════════════════════════════════
    const outcome = (typeof _flHealthLastOutcome === 'function') ? _flHealthLastOutcome() : null;
    const failed  = (typeof _flHealthLastFailures === 'function') ? _flHealthLastFailures() : [];
    // 30 days is the cadence for a PASSING run — it is not a grace period that
    // a failure ages out of. A failed check is current until something clears it.
    const aged    = days == null || days >= 30;

    let dot, strong, text, tail;
    if (outcome === false) {
      dot = 'var(--danger)'; strong = 'var(--danger)';
      text = `last run FAILED (${when})`;
      tail = failed.length
        ? `${failed.length} check${failed.length === 1 ? '' : 's'} failed: ${failed.join(', ')} — re-run for detail`
        : 'at least one check failed — re-run for detail';
    } else if (outcome === true) {
      dot = aged ? 'var(--warn)' : 'var(--success)';
      strong = aged ? 'var(--warn)' : 'var(--silver-lt)';
      text = `all clear · ${when}`;
      tail = aged ? 'older than the 30-day cadence — worth re-running' : 'reads only, writes nothing';
    } else {
      dot = 'var(--warn)'; strong = 'var(--warn)';
      text = last == null ? when : `${when} · outcome not recorded`;
      tail = 'no verdict on record — run it to find out';
    }

    el.innerHTML =
      `<span style="color:${dot}">●</span>` +
      `<span>Data health checks: <strong style="color:${strong}">${text}</strong></span>` +
      ...
      `<span style="color:var(--faint)">${tail}</span>`;
  })();
