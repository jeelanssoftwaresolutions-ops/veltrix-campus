...
async function renderHidden() {
  ...
  try {
    ...
    // JSS-REF-VELTRIX-2026-003 ITEM 05.3 FIX: same self-heal as Terminated — recompute
    // totalDue with the current-year-scoped snapshot and patch stale records in place.
    try {
      await Promise.all(list.map(async r => {
        if (!r.studentId) return;
        const s2 = await _computeCurrentYearFeeSnapshot(r.studentId);
        if (s2.totalDue !== (r.totalDue || 0)) {
          r.totalDue = s2.totalDue;
          schoolCol('hiddenStudents').doc(r.id).update({ totalDue: s2.totalDue }).catch(()=>{});
        }
      }));
    } catch(_) { /* non-fatal */ }

    // ITEM 05.2 FOLLOW-ON: also self-heal amountPaid/outstandingBalance — the unified
    // engine writes feeTransactions + students/{id} only, never hiddenStudents/{id}
    // directly, so these two columns need the same live-recompute treatment as totalDue.
    try {
      await Promise.all(list.map(async r => {
        if (!r.studentId) return;
        const s2 = await _computeCurrentYearFeeSnapshot(r.studentId);
        const patch = {};
        if (s2.totalDue !== (r.totalDue || 0)) patch.totalDue = s2.totalDue;
        if (s2.totalPaid !== (r.amountPaid || 0)) patch.amountPaid = s2.totalPaid;
        if (s2.outstanding !== (r.outstandingBalance || 0)) patch.outstandingBalance = s2.outstanding;
        if (Object.keys(patch).length) {
          Object.assign(r, patch);
          schoolCol('hiddenStudents').doc(r.id).update(patch).catch(()=>{});
        }
      }));
    } catch(_) { /* non-fatal */ }
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Parent</th><th>Contact</th><th>Total Paid</th><th>Outstanding</th><th>Status</th><th>Added</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                ${list.length === 0
                  ? `<tr><td colspan="12" style="text-align:center;padding:40px;color:var(--muted)">No students in Hidden section.</td></tr>`
                  : list.map(s => {
                      const outstanding = s.outstandingBalance || 0;
                      return `
                      <tr>
                        ...
                        <td style="display:flex;gap:6px;align-items:center;padding:10px 14px">
                          ${outstanding > 0 && s.studentId
                            ? `<button class="btn btn-primary btn-sm" onclick="pushNav('recordFee',{studentId:'${s.studentId}',studentName:'${(s.studentName||'').replace(/'/g,"\\'")}',classSection:'${(s.class||'')+'  –  Section '+(s.section||'')}' })">Pay Dues</button>`
                          ...
                          <button class="btn btn-sm" style="background:rgba(82,200,122,0.12);color:var(--success);border:1px solid rgba(82,200,122,0.3);font-size:11px" onclick="undoHiddenStudent('${s.id}','${(s.studentName||'').replace(/'/g,"\\'")}')">↩ Undo Hidden</button>
                          <button class="btn btn-sm" style="background:rgba(224,82,82,0.12);color:var(--danger);border:1px solid rgba(224,82,82,0.3);font-size:11px" onclick="removeHiddenStudent('${s.id}','${(s.studentName||'').replace(/'/g,"\\'")}')">🗑 Remove</button>
                        </td>
                      </tr>`;
                    }).join('')
                }
  ...
  showConfirm(
    ...
    async () => {
      try {
        ...
        // Compute financials — current-academic-year-only, excused-aware (Item 05.3 fix)
        const { totalPaid, outstanding, totalDue } = await _computeCurrentYearFeeSnapshot(id);

        await schoolCol('hiddenStudents').add({
          studentId:       id,
          studentName:     s.name,
...
async function exportHiddenPDF() {
  ...
  const list = window._allHidden;
  if (!list.length) { showToast('No hidden student records found.','warning'); return; }
  exportSimplePDF('Hidden Students — CONFIDENTIAL',
    ['Block','Name','Adm#','Class','Section','Parent','Contact','Total Paid','Outstanding','Status'],
    list.map(s=>[
      s.block||'—', s.studentName, s.admissionNumber, s.class, s.section,
      s.parentName||'—', s.contact||'—',
      '₹'+fmtNum(s.amountPaid||0), '₹'+fmtNum(s.outstandingBalance||0),
      (s.outstandingBalance||0)>0?'Dues Pending':'Cleared'
    ]));
}
...
async function exportHiddenExcel() {
  ...
  if (!list.length) { showToast('No hidden student records found.','warning'); return; }
  exportXLS([
    ['Block','Name','Adm#','Class','Section','Parent','Contact','Total Paid','Outstanding','Status','Added On'],
    ...list.map(s=>[
      s.block||'—', s.studentName, s.admissionNumber,
      s.class, s.section, s.parentName||'—', s.contact||'—',
      s.amountPaid||0, s.outstandingBalance||0,
      (s.outstandingBalance||0)>0?'Dues Pending':'Cleared',
      fmtDate(s.addedAt)
    ])
