    ...
    try {
      await Promise.all(list.map(async r => {
        ...
        if (s2.totalDue !== (r.totalDue || 0)) {
          ...
          _flHealSnapshotDoc('hiddenStudents', r.id, { totalDue: s2.totalDue });   // F2
        }
      }));
    } catch(_) { /* non-fatal */ }
  ...
  showConfirm(
    ...
    async () => {
      try {
        ...
        // failed un-hide. Rahul Kumar went through six hide/undo cycles in one evening;
        // every one relied on the delete landing.
...
async function exportHiddenPDF() {
  ...
  exportSimplePDF('Hidden Students — CONFIDENTIAL',
    ...
    list.map(s=>[
      ...
      'INR '+fmtNum(s.amountPaid||0), 'INR '+fmtNum(s.outstandingBalance||0),
