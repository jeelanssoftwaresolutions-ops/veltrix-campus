                      ...
                      <tr>
                        ...
                        <td style="display:flex;gap:6px;align-items:center;padding:10px 14px">
                          ${duesAllYears > 0 && s.studentId
                            /* fromArchive is the entitlement _flPaymentGuard checks.
                               Hidden students are now blocked at every other door, so
                               THIS screen — principal-only in its own right — has to
                               say who it is, exactly as Terminated's Pay Dues does. */
                            ? `<button class="btn btn-primary btn-sm" onclick="pushNav('recordFee',{studentId:'${s.studentId}',studentName:'${jsAttr(s.studentName)}',classSection:'${(s.class||'')+'  –  Section '+(s.section||'')}',fromArchive:true })">Pay Dues</button>`
  ...
  showConfirm(
    ...
    async () => {
      try {
        ...
        // Their EXISTING receipts become confidential too. isHiddenPayment was only
        // ever stamped on payments written while already hidden, so without this the
        // student's whole payment history stayed visible to Admin — and was missing
        // from the Principal's own Hidden Fee History, which queries on that flag.
        const _vis = await _flSetPaymentVisibility(id, 'isHiddenPayment', true);
        if (!_vis.ok) {
          showToast(`${name} was moved to Hidden, but their existing receipts could NOT ` +
                    `be marked confidential (${_vis.error}). Those payments remain visible ` +
                    `to Admin. Undo Hidden and try again.`, 'danger');
        }
  ...
  showConfirm(
    ...
    async () => {
      try {
        ...
        // Confidentiality is lifted from their receipts too. Leaving the flag set
        // would hide an ACTIVE student's payments from Finance, Export and the
        // dashboard totals — the school's own collection figures would be short by
        // this student's history, with nothing on screen to explain the gap.
        const _visU = await _flSetPaymentVisibility(studentId, 'isHiddenPayment', false);
        if (!_visU.ok) {
          showToast(`${name} was restored, but their receipts are still marked ` +
                    `confidential (${_visU.error}). Their payments will stay missing ` +
                    `from Finance and dashboard totals until this is retried.`, 'danger');
        }
