...
async function doLogout() {
  ...
  navStack = [];
  // Cleared with the stack it belongs to — a student id left here across a logout
  // would be the next account's first render. Same cross-tenant discipline as the
  // caches above.
  currentParams = {};
  _setTerminatedUnlocked(false); // BUG-M10 FIX: clear sessionStorage lock on logout
  // Remove overlay screens if visible.
  // The #timeLockScreen teardown that stood here is gone with the school-hours
  // lock: nothing creates that element any more, so removing it was searching
  // the DOM on every logout for a node that cannot exist.
