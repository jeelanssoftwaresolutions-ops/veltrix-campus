      ...
      if (typeof _computeAllYearsFeeSnapshot === 'function') {
        await Promise.all(window._allTerminated.map(async r => {
          ...
          try {
            ...
            // Through _flSnapshotPatch, the same mapping every other heal uses.
            // In-memory only here — the export must not write to the archive — so the
            // patch is applied to the row and never persisted.
...
async function _defaulterRows() {
  ...
  (window._allStudents || []).forEach(s => {
    if (!s || _flIsDeparted(s)) return;   // same exclusion Due Fee uses
...
async function exportTerminatedPDF() {
  ...
  exportSimplePDF('Terminated Students',
    ['Block','Name','Adm#','Parent','Contact','Class','Termination Date','Total Due (all yrs)','Paid','Outstanding (this yr)','Status'],
    list.map(s=>[
      s.block||'—',
      s.studentName,
      s.admissionNumber,
      ...
      (s.totalDue||0)>0?'Dues Pending':'Cleared'   // all-years dues, not the current-year slice
    ]));
...
async function exportTerminatedExcel() {
  ...
  exportXLS([
    ['Block','Name','Adm#','Parent','Contact','Class','Section','Termination Date','Total Due (all yrs)','Amount Paid','Outstanding (this yr)','Status'],
    ...list.map(s=>[
      s.block||'—',
      s.studentName,
      s.admissionNumber,
