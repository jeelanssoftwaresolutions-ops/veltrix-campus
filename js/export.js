...
async function exportFinanceExcel() {
  ...
  try {
    ...
    const txs = window._financeData || [];
    if (!txs.length) { showToast('No transaction data available. Visit the Finance page first.', 'warning'); return; }
    ...
    const rows = txs.map(t=>([t.studentBlock||t.block||'—',t.receiptNumber,t.studentName,t.parentName||'—',t.contactNo||t.phone||'—',t.admissionNumber,t.classSection,t.feeHead,t.amountPaid,t.remainingBalance,t.paymentMode,_getChequeNoDisplay(t)||'',fmtDate(t.date),t.paymentStatus,t.recordedByName]));
    exportXLS([['Block','Receipt#','Student','Parent','Contact','Adm#','Class','Fee Head','Amount','Balance','Mode','Cheque No.','Date','Status','Recorded By'],...rows], 'Paid_Fee');  // [CHG-008]
  } finally {
...
async function exportFinancePDF() {
  ...
  try {
    ...
    exportSimplePDF('Paid Fee Report',  // [CHG-008] renamed from Fee Collection Report
      ['Block','Receipt#','Student','Parent','Contact','Class','Fee Head','Amount','Mode','Cheque No.','Date','Status'],
      txs.map(t=>[t.studentBlock||t.block||'—',t.receiptNumber,t.studentName,t.parentName||'—',t.contactNo||t.phone||'—',t.classSection,t.feeHead,'₹'+fmtNum(t.amountPaid),t.paymentMode,_getChequeNoDisplay(t)||'—',fmtDate(t.date),t.paymentStatus]));
  } finally {
