...
async function saveEditStudent(id) {
  ...
  if (isPrincipalSave && admDateRaw) {
    // Date-only fact — 12:00 IST via the shared converter, not UTC midnight.
    data.admissionDate = istTimestampFromDateInput(admDateRaw, 'noon');
  }
  ...
  try {
    // ITEMS 4 + 17: an ordinary profile edit is not a fee event — but this form can
    // change CLASS, and every rate resolution in the app starts from the class. That
    // made "fix a typo in the class" a silent re-pricing: the rate changed, nothing
    // recomputed, and outstandingBalance kept its old-class value on every screen.
    // _flFeeRelevantChange keeps name/contact/section edits to a single write.
    let _feeChanged = false;
    try {
      const _beforeSnap = await schoolCol('students').doc(id).get();
      _feeChanged = (typeof _flFeeRelevantChange === 'function')
        ? _flFeeRelevantChange(_beforeSnap.exists ? _beforeSnap.data() : {}, data) : false;
    } catch(_) { _feeChanged = true; }   // cannot tell -> reconcile, never skip on doubt

    await schoolCol('students').doc(id).update(data);

    if (_feeChanged && typeof _flReconcile === 'function') {
      await _flReconcile(id, 'student_edit_class_change');
      if (typeof invalidateFinanceCache === 'function') invalidateFinanceCache();
    }
    auditLog('student_profile_edited', { studentId: id, updatedFields: Object.keys(data), feeRelevant: _feeChanged }); // BUG-N19 FIX
