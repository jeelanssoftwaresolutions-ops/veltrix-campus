...
function _ffPaymentModeTotals(txs) {
  ...
  (txs || []).forEach(t => {
    ...
    // ATTRIBUTION — which mode(s) the money arrived by. The breakup is used here and
    // ONLY here; it records the amount tendered per mode.
    ...
    // TOTAL — always the shared definition, never the breakup. Summing the breakup
    // here is what made this bar disagree with the year buckets and with Dashboard
    // on any partial payment (tendered != applied).
    total += _txCollectedAmount(t);
...
async function renderFinance(startAfterDoc=null) {
  ...
  try {
    // DECISION A (accuracy over convenience): Paid Fee is used for daily reconciliation,
    // so its figures must be the TRUE totals — not "of the loaded page". This previously
    // read only PAGE_SIZE=75 rows, so past 75 transactions both the summary bar and the
    // per-year buckets silently under-reported against Dashboard, which listens to the
    // whole collection. The v8 compat SDK in use here has no server-side aggregate/sum
    // query (that is v9-modular only), so the honest option is to read the collection —
    // exactly what Dashboard already does on every visit, so this adds no new class of
    // cost. Pagination of the rendered rows is no longer needed as a result.
    const PAGE_SIZE = 75;   // retained only for loadMoreFinance()'s legacy signature
    const snap = await schoolCol('feeTransactions').orderBy('date','desc').get();
    const txs  = snap.docs.map(d=>_normalizeTx({id:d.id,...d.data()})).filter(t => currentRole === 'principal' || !t.isHiddenPayment);
    const hasMore = false;  // everything is loaded, so totals always equal Dashboard's
    ...
    const renderRows = (rows) => {
      ...
      return yearEntries.map(([yr, yrRows], idx) => {
        ...
        const yrTotal    = _txCollectedTotal(yrRows);   // shared definition
              ...
              <div class="card" style="margin-bottom:16px;border-color:${palette.border};background:${palette.bg}">
                <div class="card-hdr" style="cursor:pointer;user-select:none;display:flex;justify-content:space-between;align-items:center"
                  ...
                  <div style="display:flex;align-items:center;gap:14px">
                    ...
                    <!-- ITEM 11: chevron matches the now-always-collapsed initial state -->
                    <span id="${bodyId}_chevron" style="font-size:16px;color:var(--muted);transition:transform 0.2s;display:inline-block;transform:rotate(-90deg)">▾</span>
                  </div>
                </div>
                <!-- ITEM 11: every academic-year bucket starts COLLAPSED. The current year
                     used to auto-expand on load, so the section appeared to open itself. -->
    ...
    setContent(`
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:0">
          <!-- ITEM 2: tbl-sticky keeps the column header pinned directly above the rows -->
...
function _pfPaidYrSearch(bodyId) {
  ...
  const filteredTotal = _txCollectedTotal(filtered);   // shared definition
