...
function _renderCsRows(records, allStudents, feeSchedule, concMap, filters={}) {
  ...
  return rows.map(r => {
    ...
    return `
      <tr>
        <td><strong>${sanitizeHTML(r.studentName || '—')}</strong></td>
...
async function saveFeeStructure() {
  ...
  try {
    ...
    await db.collection('schools').doc(currentSchoolId).update({
      'config.feeSchedule': newSchedule,
      ...feeCatUpdate,
      updatedAt: firebase.firestore.FieldValue.serverTimestamp()
    });

    // Update in-memory schedule so _FEE_SCHEDULE Proxy reflects changes immediately
    _tenantFeeSchedule = newSchedule;
    ...
    const msg = e.code === 'permission-denied'
      ? '❌ Permission denied. Firestore Rules must allow Principal to write schools/{id}.config.feeSchedule.'
