  ...
  try {
    ...
    if ((!studentYearNorm || studentYearNorm === _curYrNorm) && sData.monthStatus && typeof sData.monthStatus === 'object') {
      Object.entries(sData.monthStatus).forEach(([m, st]) => {
        if (st === 'N/A-PAID' || st === 'PAID') paidFullNames.add(_shortToFull[m] || m);
      });
    }
      ...
      <tr ${_studentRowAttrs(r)}>
        ...
        <td>
          ${expired
            ...
            onclick="openConcessionEdit('${r.id}','${(r.studentName||'').replace(/'/g,"\'")}','${r.admissionNo||''}','${r.class||r.cls||''}',${conFee},${stdFee})">✏️ Edit</button>`}
          <button class="btn btn-sm" style="background:rgba(224,82,82,0.12);color:var(--danger);border:1px solid rgba(224,82,82,0.3);font-size:11px;margin-left:4px"
            onclick="removeConcession('${r.id}','${(r.studentName||'').replace(/'/g,"\'")}')">✕ Remove</button>
        </td>
      </tr>`;
  }).join('');
}
  ...
  modal.innerHTML = `
    <div style="background:var(--panel);border:1px solid var(--glass-border);border-radius:14px;padding:28px 32px;min-width:340px;max-width:460px;max-height:90vh;overflow-y:auto">
      ...
      <div style="display:flex;gap:10px;margin-bottom:${activeMonths.length ? '20px' : '4px'}">
        <button class="btn btn-primary btn-sm" onclick="saveConcessionEdit('${docId}','${sanitizeHTML(studentName)}','${admissionNo}','${cls}',${stdFee})">Save Default</button>
      </div>
      ...
      <div style="padding-top:16px;border-top:1px solid var(--glass-border)">
        ...
        <div style="display:flex;gap:10px;margin-bottom:4px">
          <button class="btn btn-primary btn-sm" onclick="saveConcessionMonthEdit('${docId}','${sanitizeHTML(studentName)}',${stdFee})">Save Month</button>
          <button class="btn btn-ghost btn-sm" onclick="resetConcessionMonthEdit('${docId}','${sanitizeHTML(studentName)}')">Reset to Default</button>
        </div>` : `
...
async function saveConcessionMonthEdit(docId, studentName, stdFee) {
  ...
  try {
    ...
    try { auditLog('concession_month_edit', { studentName, month: key, amount: val }); } catch(_) {}
...
async function removeConcession(docId, studentName) {
  ...
  try {
    ...
    if (!_admNo) {
      try { const d = await schoolCol('concessionFees').doc(docId).get();
            _admNo = (d.exists && (d.data().admissionNo || d.data().admNo)) || ''; } catch(_) {}
    }
    await schoolCol('concessionFees').doc(docId).delete();
    await _flReconcileByAdmissionNo(_admNo, 'concession_removed');
    invalidateFinanceCache(); invalidateStudentCache();
