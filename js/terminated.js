...
async function renderTerminated() {
  ...
  try {
    ...
    const tbodyHtml = list.map(s => {
      ...
      return `<tr>
        <td><strong style="cursor:pointer;color:var(--text)" onclick="${s.studentId ? `renderStudentProfile('${s.studentId}')` : ''}" title="${s.studentId ? 'View fee profile' : ''}">${s.studentName||'—'}</strong>${legacyBadge}</td>
