...
//
// totalDue is defined as paid + excused + outstanding, so the three columns shown on
// the Terminated table add up to it exactly.
// ════════════════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════════════════
...
function _computeAllYearsFeeSnapshot(studentId) {
  return Promise.all([
    ...
    // Authoritative all-years figure. Fall back to the newest transaction's
    // remainingBalance only when the student doc carries no aggregate at all.
    let outstanding = Number(s.outstandingBalance);
    if (!Number.isFinite(outstanding)) {
      const withBal = txs.filter(t => t.type !== 'excused_waiver' && typeof t.remainingBalance === 'number')
                         .sort((a, b) => (b.date?.seconds || 0) - (a.date?.seconds || 0));
      outstanding = withBal.length ? (withBal[0].remainingBalance || 0) : 0;
    }
    outstanding = Math.max(0, outstanding);

    return { totalPaid, excusedTotal, outstanding, studentMissing,
             totalDue: totalPaid + excusedTotal + outstanding };
  });
}
