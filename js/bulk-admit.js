  ...
  try {
    ...
    rows.forEach((r, i) => {
      ...
      valid.push({
        ...
        outstandingBalance: parseFloat(r.OutstandingBalance||0) || (monthsDue * monthlyFee),
        monthStatus:     monthStatus,   // [CHG-016] DueTracker month grid
        totalDue:        parseFloat(r.TotalDue||0) || (monthsDue * monthlyFee),
        monthsDue:       monthsDue,
        ...
        // [CHG-016] fee_status derived from monthsDue (v2.0 logic)
        fee_status: monthsDue > 0 ? 'pending' : 'paid',
...
function _deduplicateRowsInSheet(rows) {
  ...
  for (const r of rows) {
    ...
    _accum.forEach(e => {
      const y = _yrKey(e && e.year);
      if (!y) return;
      if (y === _yrKey(older.academicYear) || y === _yrKey(newer.academicYear)) return;
      _byYear.set(y, e);
    });
    const _mergedOpeningDues = Array.from(_byYear.values());

    map.set(key, Object.assign({}, newer, {
      outstandingBalance:      combinedOutstanding,
      totalDue:                combinedTotalDue,
      fee_status:              combinedOutstanding > 0 ? 'pending' : 'paid',
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (existingDoc) {
        ...
        // about.
        // ══════════════════════════════════════════════════════════════════════
        const _yrK = y => String(y || '').trim();
        const _oodMap = new Map();
        (Array.isArray(existData.openingOutstandingDues) ? existData.openingOutstandingDues : [])
          .forEach(e => { const y = _yrK(e && e.year); if (y) _oodMap.set(y, e); });
        (Array.isArray(r.openingOutstandingDues) ? r.openingOutstandingDues : [])
          .forEach(e => { const y = _yrK(e && e.year); if (y) _oodMap.set(y, e); });
        const _scalarYear = _yrK(r.previousAcademicYear || existData.previousAcademicYear);
        const _curYearBA  = _yrK(r.academicYear);
        [_scalarYear, _curYearBA].forEach(y => { if (y) _oodMap.delete(y); });
        const _oodMerged = Array.from(_oodMap.values());
