...
async function parseBulkAdmitFile(input) {
  ...
  try {
    ...
    rows.forEach((r, i) => {
      ...
      const monthlyFee = parseFloat(r.MonthlyFee || r['MonthlyFee(Rs.)'] || '0') || _FEE_SCHEDULE[cls] || 0;
      valid.push({
        ...
        academicYear:    String(r.AcademicYear||'').trim() || _getCurrentAcademicYearStr(),
        ...
        admissionDate: (() => {
          ...
          if (_rawAdmDate) return firebase.firestore.Timestamp.fromDate(new Date(_rawAdmDate));
          ...
          if (_startYr && !isNaN(_startYr)) {
            ...
            return firebase.firestore.Timestamp.fromDate(new Date(_startYr, 5, 1)); // month 5 = June
          }
        ...
        dueDate: (() => {
          ...
          const _startYr = _prevYr ? parseInt(_prevYr.split('-')[0], 10) : null;
          if (_startYr && !isNaN(_startYr)) {
            return firebase.firestore.Timestamp.fromDate(new Date(_startYr, 5, 1));
          }
          return firebase.firestore.FieldValue.serverTimestamp();
        })(),
        status: 'active',
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (existingDoc) {
        ...
        // Accumulate if Firestore record already has some outstanding balance
        const firestoreOutstanding = existData.outstandingBalance || 0;
        const sheetOutstanding     = r.outstandingBalance || 0;
        // Avoid double-counting: only add if the academicYear changed
        const combinedOutstanding  = existData.academicYear !== r.academicYear
