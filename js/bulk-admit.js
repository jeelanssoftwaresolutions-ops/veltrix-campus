...
// ════════════════════════════════════════════════════════════════════════════
// A DATE OUT OF A SPREADSHEET IS NOT A DATE STRING.
//
// istInstantFromDateInput accepts only /^\d{4}-\d{2}-\d{2}/. Anything else
// returns null, and istTimestampFromDateInput then falls back to `new Date()` —
// deliberately, so a record never carries epoch zero. The consequence on this
// path was that an unparseable admission date was silently replaced by TODAY.
//
// And .xlsx never hands over a date string. XLSX.read without cellDates gives an
// Excel SERIAL NUMBER (45900), so `String(r.AdmissionDate)` was "45900", which
// matched nothing, and every imported student was stamped as admitted on the day
// of the import. The same sheet saved as .csv worked, because CSV yields text —
// one file, two formats, two different admission dates, no warning either way.
//
// Everything the import can actually receive is resolved here: a JS Date (with
// cellDates on), a serial as number or numeric string, ISO, and dd/mm/yyyy.
// Anything else returns '' and the caller raises a row error rather than
// inventing a date — the same rule F1 applied to money.
//
// dd/mm/yyyy is read day-first. That is the convention these sheets are written
// in, and the app's own inputs are ISO, so there is no mm/dd source to confuse
// it with. A value where both parts are <= 12 is genuinely ambiguous and is read
// day-first too; that assumption is stated here rather than buried.
// ════════════════════════════════════════════════════════════════════════════
function _baSheetDateToISO(v) {
  const p2 = n => String(n).padStart(2, '0');
  if (v == null || v === '') return '';

  if (v instanceof Date && !isNaN(v.getTime())) {
    // SheetJS builds these off the workbook's own epoch; read UTC so a browser
    // timezone cannot shift the calendar day.
    return v.getUTCFullYear() + '-' + p2(v.getUTCMonth() + 1) + '-' + p2(v.getUTCDate());
  }

  // Excel serial -> calendar date, done here rather than through XLSX.SSF. That
  // helper is a SheetJS internal, it is absent from the contract-test harness, and
  // depending on it made this branch both untestable and hostage to a library's
  // private API. The arithmetic is fixed and short.
  //
  // Day 0 is 1899-12-30, not 1900-01-01, which absorbs Excel's deliberate 1900
  // leap-year bug for every date after 1900-03-01. Serials below 61 fall inside
  // that bug and are rejected — an admission date in January 1900 is a corrupt
  // cell, not a record.
  const asSerial = n => {
    if (!isFinite(n) || n < 61) return '';
    const ms = Date.UTC(1899, 11, 30) + Math.floor(n) * 86400000;
    const d  = new Date(ms);
    if (isNaN(d.getTime())) return '';
    return d.getUTCFullYear() + '-' + p2(d.getUTCMonth() + 1) + '-' + p2(d.getUTCDate());
  };
  if (typeof v === 'number') return asSerial(v);
  ...
  if (dmy) {
    ...
    if (d >= 1 && d <= 31 && m >= 1 && m <= 12) return dmy[3] + '-' + p2(m) + '-' + p2(d);
    return '';
  }
  if (/^\d+(\.\d+)?$/.test(s)) return asSerial(Number(s));   // serial arrived as text
  return '';
}
...
async function parseBulkAdmitFile(input) {
  ...
  try {
    ...
    if (ext === 'csv') {
      // ══════════════════════════════════════════════════════════════════════
      // A CSV IS NOT A STRING SPLIT ON COMMAS.
      //
      // This read `line.split(',')`, with no quoting and no embedded-newline
      // handling. One comma inside a quoted field — "Kapoor, Kabir", a parent
      // name, an address — shifts every column after it by one. Silently: the
      // row still imports, the parent's name lands in the class column, and a
      // money column takes whatever was to its left.
      //
      // It never announced itself because nothing downstream can tell a shifted
      // row from a badly filled one, and the sheet that produced it looks
      // correct when reopened. .xlsx was unaffected — SheetJS parses properly —
      // so the same data imported cleanly or corrupted depending on which format
      // the office happened to save.
      //
      // Proper RFC-4180 scan: quotes toggle, "" inside a quoted field is a
      // literal quote, and a newline inside quotes belongs to the field rather
      // than ending the record.
      // ══════════════════════════════════════════════════════════════════════
      ...
      const headers = (_csvRows[0] || []).map(h => String(h).trim());
      for (let i = 1; i < _csvRows.length; i++) {
        const vals = _csvRows[i].map(v => String(v).trim());
        const row  = {};
        // Index, not `|| ''` — a legitimate "0" must survive, the same rule F1
        // established for the money columns.
        headers.forEach((h, idx) => { row[h] = (vals[idx] === undefined ? '' : vals[idx]); });
        rows.push(row);
      }
    } else {
      const data    = await file.arrayBuffer();
      // cellDates: a date cell otherwise arrives as an Excel SERIAL NUMBER, which no
      // date parser in this app accepts. _baSheetDateToISO still handles serials for
      // workbooks this flag cannot help with, but asking for Dates is the cheaper half.
      ...
      // ══════════════════════════════════════════════════════════════════════
      // EVERY SHEET IS READ, AND THE HEADER ROW IS FOUND RATHER THAN GUESSED.
      //
      // Two problems, both silent:
      //
      // ONE SHEET  only wb.SheetNames[0] was read. A workbook carrying a year per
      //            sheet — which is exactly how a multi-year roll arrives, one tab
      //            per academic year up to the current grade — imported only its
      //            first tab, and a cover sheet in slot one imported NOTHING. No
      //            error either way: the preview simply showed fewer rows than the
      //            file contained, and nobody counts rows in a bulk import.
      //
      // BANNER     the header row was located by matching cell A1 against 'veltrix',
      //            'bulkadmit' or a school emoji. Any other banner text meant row 1
      //            was read AS the headers, so every real header became a data value
      //            and the whole sheet failed validation for missing StudentName.
      //
      // The header row is now found by LOOKING FOR HEADERS: the first of the first
      // few rows that maps to at least two known v2.0 columns wins. That works
      // whatever the banner says, in any language, and needs no maintenance when
      // the banner text changes.
      //
      // A sheet with no recognisable header row is skipped rather than failed —
      // a workbook is allowed to contain notes, a summary or an empty tab beside
      // the data. If NO sheet yields headers the caller's own validation reports
      // it, which is where that message belongs.
      // ══════════════════════════════════════════════════════════════════════
      const _remap = r => {
        const mapped = {};
        Object.entries(r).forEach(([k, v]) => { mapped[HEADER_MAP[k] || k] = v; });
        return mapped;
      };
      const _KNOWN = Object.keys(HEADER_MAP).concat([
        'StudentName','AdmissionNo','Class(Prev)','Class(Curr)','Section','Block','Gender',
        'ParentName','ContactNumber','AcademicYear','MonthlyFee','MonthsPaidBeforePromotion',
        'PromotionStatus','OutstandingBalance','TotalDue','MonthsDue','MonthsCleared','AdmissionDate',
      ]).map(k => String(k).trim());
      const _looksLikeHeaderRow = obj => {
        const keys = Object.keys(_remap(obj)).map(k => String(k).trim());
        return keys.filter(k => _KNOWN.indexOf(k) > -1).length >= 2;
      };
      ...
      wb.SheetNames.forEach(name => {
        ...
        if (!ws) return;
        // Try the first few offsets — a banner may be one line or several.
        for (let off = 0; off <= 4; off++) {
          let candidate;
          try { candidate = XLSX.utils.sheet_to_json(ws, { defval: '', range: off }); }
          catch (_) { break; }
          if (!candidate || !candidate.length) continue;
          if (!_looksLikeHeaderRow(candidate[0])) continue;
          candidate.forEach(r => _sheetRows.push(_remap(r)));
          _sheetsUsed.push(name);
          if (off > 0) hasBanner = true;    // preserved for the caller's messaging
          break;
        }
      });
      if (_sheetsUsed.length > 1) {
        console.log('[BULK ADMIT] read ' + _sheetsUsed.length + ' sheets: ' + _sheetsUsed.join(', '));
      }
      rows = _sheetRows;
    }
    ...
    rows.forEach((r, i) => {
      ...
      // A date the sheet states must survive the import or stop it. Blank is fine —
      // the fallbacks below derive one from the academic year. A value that is
      // PRESENT and unreadable is a row error, because the alternative is what this
      // path used to do: silently stamp today and let the admission-entry filters
      // report the whole import as admitted on upload day.
      const _admRaw = (r.AdmissionDate === 0 || r.AdmissionDate) ? r.AdmissionDate : '';
      const _admISO = _baSheetDateToISO(_admRaw);
      if (String(_admRaw).trim() !== '' && !_admISO) {
        errors.push(`Row ${rowNum} (${r.StudentName||'unnamed'}): AdmissionDate "${_admRaw}" could not be read. ` +
                    `Use YYYY-MM-DD or DD/MM/YYYY, or leave it blank to derive it from the academic year.`);
        return;
      }
      ...
      valid.push({
        ...
        admissionDate: (() => {
          // _admISO is already normalised — a Date, an Excel serial and a dd/mm string
          // all arrive here as YYYY-MM-DD, which is the only form the converter reads.
          if (_admISO) return istTimestampFromDateInput(_admISO, "noon");
